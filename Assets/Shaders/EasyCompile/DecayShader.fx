...
float4 PixelShaderFunction(float2 coords : TEXCOORD0) : COLOR0
{
    ...
    float2 noiseCoords = coords + float2(uTime * 0.5 * prog, uTime * 0.1 * prog);
    ...
    float4 finalColor = shapeColor * float4(uColor.rgb, uOpacity);
    ...
    float fadeStruct = smoothstep(0.3, 1.0, dist);
    
    float opacityMask = 1.0 - fadeStruct;
    
    finalColor *= opacityMask;
    
    finalColor.g *= (1.0 - smoothstep(0.0, 0.5, dist));
