...
{
    public class BeekeeperPlayer : ModPlayer
    {
        ...
        public override void PostUpdateMiscEffects()
        {
            ...
            if (HasBees)
            {
                ...
                if (HoneyShieldCD == 1)
                {
                    ...
                    Projectile.NewProjectile(Player.GetSource_FromThis(), Player.Center, Vector2.UnitY * -5f, ProjectileType<BeePlayerShieldProjectile>(), 0, 0f, Player.whoAmI);
                }
            }
        ...
        public override bool ImmuneTo(PlayerDeathReason damageSource, int cooldownCounter, bool dodgeable)
        {
            if (CurrentBeeState == (int)BeeState.Defense)
            {
                if (HoneyShield)
                {
                    ...
                    BeeUtils.CircleDust(Player.Center, 50, DustType<HoneyMetaballDustTransparent>(), 3.5f, 0, null, 2.5f);
                }
        ...
        private void UpdateDefense()
        {
            ...
            if (HoneyShieldCD == 1)
                BeeUtils.CircleDust(Player.Center, 50, DustType<HoneyMetaballDustTransparent>(), 3.75f, 0, null, 2f);
        }
            ...
            if (CurrentBeeState == (int)BeeState.Defense)
            {
                ...
                else
                {
                    ...
                    Filters.Scene["CircleDistort"].GetShader().
                        ...
                        UseImage(Request<Texture2D>("BombusApisBee/ExtraTextures/SwirlyNoiseLooping").Value, 1).
                        UseImage(Request<Texture2D>("BombusApisBee/ExtraTextures/MiscNoise1").Value, 2).
    ...
    public class BeePlayerBeeProjectile : ModProjectile
    {
        ...
        public bool Defense => Player.Beekeeper().CurrentBeeState == (int)BeekeeperPlayer.BeeState.Defense;
        public bool Offense => Player.Beekeeper().CurrentBeeState == (int)BeekeeperPlayer.BeeState.Offense;
        public bool Gathering => Player.Beekeeper().CurrentBeeState == (int)BeekeeperPlayer.BeeState.Gathering;
        ...
        public override bool PreAI()
        {
            ...
            if (Gathering && stuck)
            {
                ...
                if (++HoneyTimer % 150 == 0)
                    Projectile.NewProjectileDirect(Projectile.GetSource_FromAI(), Projectile.Center + Projectile.velocity, Projectile.DirectionTo(Player.Center) * 10f, ProjectileType<BeeResourceIncreaseProjectile>(), 0, 0f, Player.whoAmI, 0, Player.Beekeeper().GatheringIncrease).tileCollide = false;
        ...
        public override void AI()
        {
            ...
            if (Defense)
            {
                ...
                if (Player.Beekeeper().JustShielded)
                    for (int i = 0; i < 5; i++)
                    {
                        Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(), Main.rand.NextVector2Circular(3f, 3f), 0, new Color(255, 205, 0), 0.45f);
                    }

                if (Player.Beekeeper().HoneyShieldCD == 1)
                    BeeUtils.CircleDust(Projectile.Center, 10, DustType<GlowFastDecelerate>(), 1f, 0, new Color(255, 205, 0), 0.5f);
                ...
                lerper = 1f - Player.Beekeeper().HoneyShieldCD / (float)Player.Beekeeper().MaxHoneyShieldCD;
            ...
            else if (Gathering)
            {
                ...
                if (attackTarget != null && AttackDelay <= 0)
                {
                    if (Main.rand.NextBool(5))
                        Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(), Vector2.Zero, 0, Main.rand.NextBool() ? new Color(255, 125, 0) : new Color(255, 200, 0), 0.25f);
        ...
        private void ManageTrail()
        {
            ...
            {
                return Color.Lerp(Color.Transparent, new Color(255, 50, 20, 0) * mult, lerper) * (Player.Beekeeper().HoldingBeeWeaponTimer / 15f);
            });
            ...
            {
                return Color.Lerp(Color.Transparent, new Color(255, 200, 0, 0) * 0.25f * mult, lerper) * (Player.Beekeeper().HoldingBeeWeaponTimer / 15f);
            });
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            if (Attacking)
            {
                if (Offense)
                {
                    ...
                    for (int i = 0; i < 5; i++)
                    {
                        Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(), (-Projectile.velocity * Main.rand.NextFloat(0.2f)).RotatedByRandom(0.55f), 0, Main.rand.NextBool() ? new Color(255, 125, 0) : new Color(255, 200, 0), 0.45f);
                    }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            lightColor = lightColor * (Player.Beekeeper().HoldingBeeWeaponTimer / 15f);

            if (Defense)
            {
                Color glowColor = Color.Lerp(Color.Transparent, new Color(255, 125, 0, 0), 1f - Player.Beekeeper().HoneyShieldCD / (float)Player.Beekeeper().MaxHoneyShieldCD) * (Player.Beekeeper().HoldingBeeWeaponTimer / 15f);
            ...
            if (Offense)
            {
                Color glowColor = Color.Lerp(Color.Transparent, new Color(255, 125, 0, 0), 1f - AttackDelay / (float)maxAttackDelay) * (Player.Beekeeper().HoldingBeeWeaponTimer / 15f);
