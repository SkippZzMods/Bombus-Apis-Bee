namespace BombusApisBee.Core.Systems.PrimitiveSystem
{
    ...
    public class SLR_Primitive : IDisposable
    {
        ...
        public SLR_Primitive(GraphicsDevice device, int maxVertices, int maxIndices)
        {
            this.device = device;
    ...
    {
        private readonly SLR_Primitive primitives;
        ...
        public Trail(GraphicsDevice device, int maxPointCount, ITrailTip tip, TrailWidthFunction trailWidthFunction, TrailColorFunction trailColorFunction)
        {
            ...
            primitives = new SLR_Primitive(device, maxPointCount * 2 + this.tip.ExtraVertices, 6 * (maxPointCount - 1) + this.tip.ExtraIndices);
        }
