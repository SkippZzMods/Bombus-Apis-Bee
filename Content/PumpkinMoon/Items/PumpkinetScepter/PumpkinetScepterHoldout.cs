...
using Terraria;
namespace BombusApisBee.Content.PumpkinMoon.Items.PumpkinetScepter
{
    public class PumpkinetScepterHoldout : BeeProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Main.myPlayer == Projectile.owner && Owner.ownedProjectileCounts[ProjectileType<PumpkinetHornet>()] <= 0)
                Projectile.NewProjectile(Projectile.GetSource_FromThis(), Main.MouseWorld, Main.rand.NextVector2CircularEdge(5f, 5f), ProjectileType<PumpkinetHornet>(), Projectile.damage, Projectile.knockBack, Projectile.owner, 0f, 0f);
        ...
        private void UpdatePlayerVisuals()
        {
            ...
            Projectile.direction = Projectile.spriteDirection = (Math.Cos(Projectile.rotation) > 0.0).ToDirectionInt();
    ...
    class PumpkinetHornet : BeeProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Projectile.localAI[0] == 0f)
            {
                ...
                for (int i = 0; i < Repeats; ++i)
                {
                    ...
                    Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(), angle2.ToRotationVector2() * 2f, 0, new Color(225, 93, 26), 0.35f);
                }
        ...
        public override void OnKill(int timeLeft)
        {
            for (int i = 0; i < 25; i++)
            {
                Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(), Main.rand.NextVector2Circular(1f, 1f), 0, new Color(209, 93, 26), 0.55f);

                Dust.NewDustPerfect(Projectile.Center, DustType<Glow>(), Main.rand.NextVector2Circular(1.5f, 1.5f), 0, new Color(226, 130, 33), 0.45f);
            }
        }
        ...
        private void Attacking(NPC target)
        {
            ...
            if (DashDelay <= 0)
            {
                ...
                for (int i = 0; i < 8; ++i)
                {
                    ...
                    Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center, angle2.ToRotationVector2() * 8f, ProjectileType<PumpkinStinger>(), Projectile.damage * 2 / 3, 2f, player.whoAmI);
                }
                ...
                {
                    float angle2 = 6.2831855f * i / Repeats;
                    Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(), angle2.ToRotationVector2() * 3f, 0, new Color(225, 93, 26), 0.35f);
                }
            }
            else if (DashDelay <= 105)
            {
                ...
                {
                    SoundEngine.PlaySound(SoundID.Item17, Projectile.Center);
                    Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center, Projectile.DirectionTo(targetCenter) * 15f, ProjectileType<PumpkinStinger>(), Projectile.damage, 2f, player.whoAmI);
                    StingerDelay = 15f;
                }
            }
        }
    ...
    public class PumpkinStinger : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (foundTarget && Projectile.timeLeft < 465 && --hitDelay <= 0)
                Projectile.velocity = (Projectile.velocity * 20f + (targetCenter - Projectile.Center).SafeNormalize(Vector2.UnitX) * 19f) / 21f;

            Projectile.rotation = Projectile.velocity.ToRotation() + MathHelper.PiOver2;

            if (Main.rand.NextBool())
            {
                Dust dust = Dust.NewDustDirect(Projectile.position, Projectile.width, Projectile.height, DustType<GlowFastDecelerate>(), newColor: new Color(225, 93, 26));
                dust.scale = 0.3f;
                dust.velocity *= 0.5f;
            }
        }
