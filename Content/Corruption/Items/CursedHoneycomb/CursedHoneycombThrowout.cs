    ...
    public class CursedHoneycombThrowout : BeeProjectile
    {
            ...
            if (Main.myPlayer == Projectile.owner)
            {
                ...
                if (--AttackDelay <= 0)
                {
                    if (owner.UseBeeResource((Main.player[Projectile.owner].HeldItem.ModItem as BeeDamageItem).honeyCost))
                    {
                        for (int i = -1; i < 2; i++)
                        {
                            Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center + Projectile.velocity, Projectile.DirectionTo(Main.MouseWorld).RotatedBy(0.3f * i) * 21f, ProjectileType<CursedTooth>(), (int)(Projectile.damage * 0.35f), 3.5f, Projectile.owner);
                        }
                        for (int i = 0; i < 4; i++)
                        {
                            Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center + Projectile.velocity, Projectile.DirectionTo(Main.MouseWorld).RotatedByRandom(0.35f) * Main.rand.NextFloat(15f, 18f), ProjectileType<CursedEye>(), (int)(Projectile.damage * 0.5f), 1.5f, Projectile.owner);
                        }
                        ...
                        for (int i = 0; i < 15; i++)
                        {
                            ...
                            Dust.NewDustPerfect(Projectile.Center + Projectile.velocity, DustType<Dusts.GlowFastDecelerate>(), Projectile.DirectionTo(Main.MouseWorld).RotatedByRandom(0.35f) * Main.rand.NextFloat(5f, 10f), newColor: new Color(97, 130, 30), Scale: Main.rand.NextFloat(0.5f, 0.65f));
                        }
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            if (Main.rand.NextBool())
                Projectile.NewProjectile(Projectile.GetSource_OnHit(target), Projectile.Center, Vector2.One.RotatedByRandom(6.28f) * 2.5f, ProjectileType<CursedBee>(), (int)(Projectile.damage * 0.5f), 0f, Projectile.owner);
        }
