    ...
    public class CorruptedBee : CommonBeeProjectile
    {
        ...
        public override void OnKill(int timeLeft)
        {
            for (int i = 0; i < 5; i++)
            {
                ...
                Dust.NewDustPerfect(Projectile.Center, DustType<Dusts.GlowFastDecelerate>(), Main.rand.NextVector2Circular(2f, 2f), 0, new Color(140, 169, 44), 0.35f);
            }
        }

        public override bool SafePreDraw(ref Color lightColor)
        {
            Texture2D texGlow = Request<Texture2D>("BombusApisBee/ExtraTextures/GlowAlpha").Value;
            Texture2D glowTex = Request<Texture2D>("BombusApisBee/ExtraTextures/BeeGlowPREHM").Value;
            if (Giant)
