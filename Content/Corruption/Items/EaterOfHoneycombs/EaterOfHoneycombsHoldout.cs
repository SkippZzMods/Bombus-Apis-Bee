namespace BombusApisBee.Content.Corruption.Items.EaterOfHoneycombs
{
    ...
    {
        public override string Texture => "BombusApisBee/Content/Corruption/Items/EaterOfHoneycombs/EaterOfHoneycombs";
        ...
        {
            return BeeUtils.PiecewiseAnimation(throwTimer / (float)maxThrowTimer, new BeeUtils.CurveSegment[]
            {
                pullback,
                throwout
            });
        ...
        public override void AI()
        {
            ...
            if (shots < 10)
            {
                ...
                if (++shootDelay >= MaxDelay)
                {
                    ...
                    if (Main.myPlayer == Projectile.owner)
                        Projectile.NewProjectile(Projectile.GetSource_FromAI(), barrelPos, Projectile.velocity * 6f, ProjectileType<CorruptedBee>(), (int)(Projectile.damage * 0.66f), 0f, owner.whoAmI);
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            for (int i = 0; i < 15; i++)
            {
                ...
                Dust.NewDustPerfect(Projectile.Center, DustType<Dusts.GlowFastDecelerate>(), Main.rand.NextVector2Circular(2.5f, 2.5f), 0, new Color(140, 169, 44), 0.45f);
            }

            if (Main.myPlayer == Projectile.owner)
                for (int i = 0; i < 4; i++)
                {
                    Projectile.NewProjectile(Projectile.GetSource_Death(), Projectile.Center, Main.rand.NextVector2CircularEdge(10f, 10f), ProjectileType<CorruptMiniEater>(), Projectile.damage / 2, 2.5f, Projectile.owner);
                }
