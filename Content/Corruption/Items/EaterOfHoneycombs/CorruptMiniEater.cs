    ...
    public class CorruptMiniEater : BeeProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Projectile.ai[0] > 30f)
            {
                ...
                for (int i = 0; i < Main.maxNPCs; i++)
                {
                    ...
                    {
                        float num267 = npc.position.X + npc.width / 2;
                        float num268 = npc.position.Y + npc.height / 2;
            ...
            if (!flag63)
            {
                num262 = Projectile.position.X + Projectile.width / 2 + Projectile.velocity.X * 100f;
                num263 = Projectile.position.Y + Projectile.height / 2 + Projectile.velocity.Y * 100f;
            }
            float num270 = 10f;
            float num271 = 0.16f;

            Vector2 vector18 = new Vector2(Projectile.position.X + Projectile.width * 0.5f, Projectile.position.Y + Projectile.height * 0.5f);
        ...
        public override void OnKill(int timeLeft)
        {
            for (int i = 0; i < 6; i++)
            {
                ...
                Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(), Main.rand.NextVector2Circular(2.5f, 2.5f), 0, new Color(140, 169, 44), 0.4f);
            }
        }
