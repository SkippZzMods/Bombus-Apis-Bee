...
namespace BombusApisBee.Content.Dungeon.Items.SpectralBeeTome
{
    public class SpectralBeeTomeHoldout : BeeProjectile
    {
        ...
        public override string Texture => "BombusApisBee/Content/Dungeon/Items/SpectralBeeTome/SpectralBeeTome";
        ...
        public void ShootBees()
        {
            ...
            if (Owner.UseBeeResource((Main.player[Projectile.owner].HeldItem.ModItem as BeekeeperWeapon).honeyCost))
            {
                ...
                for (int i = 0; i < 2; i++)
                {
                    Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center, -Vector2.UnitY.RotatedByRandom(0.35f) * Main.rand.NextFloat(5f, 10f), ProjectileType<SpectralBee>(), Projectile.damage, Projectile.knockBack, Projectile.owner);
                }

                Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center, -Vector2.UnitY.RotatedByRandom(0.45f) * Main.rand.NextFloat(10f, 15f), ProjectileType<SpectralSoul>(), Projectile.damage, Projectile.knockBack, Projectile.owner);
                ...
                for (float k = 0; k < 6.28f; k += 0.1f)
                {
                    ...
                    Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(), new Vector2(x, y) * 0.035f, 0, new Color(6, 106, 255), 0.4f);
                }

                for (int i = 0; i < 15; i++)
                {
                    Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(), Main.rand.NextVector2Circular(2f, 2f), 0, new Color(6, 106, 255), 0.45f);
                    Dust.NewDustPerfect(Projectile.Center + Main.rand.NextVector2CircularEdge(5f, 5f), DustType<GlowFastDecelerate>(), -Vector2.UnitY.RotatedByRandom(0.35f) * Main.rand.NextFloat(2.5f, 5f), 0, new Color(130, 153, 208), 0.35f);
                }
        ...
        {
            Texture2D bookTexture = Request<Texture2D>(Texture).Value;
    ...
    class SpectralSoul : BeeProjectile, IDrawPrimitive_
    {
        ...
        public override void AI()
        {
            ...
            if (Main.rand.NextBool())
                Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(), Main.rand.NextVector2Circular(1.5f, 1.5f), 0, (Main.rand.NextBool() ? new Color(149, 238, 238) : new Color(209, 246, 239)) * opacity, 0.45f);

            if (Projectile.penetrate <= 1)
            {
                Projectile.velocity = (Projectile.velocity * 25f + (Main.player[Projectile.owner].Center - Projectile.Center).SafeNormalize(Vector2.UnitX) * 25f) / 26f;
            ...
            if (Main.myPlayer == Projectile.owner)
            {
                if (Projectile.timeLeft < 220 && Projectile.Distance(Main.MouseWorld) > 50f)
                    Projectile.velocity = (Projectile.velocity * 25f + (Main.MouseWorld - Projectile.Center).SafeNormalize(Vector2.UnitX) * 25f) / 26f;
            }
        }
        ...
        public override void OnKill(int timeLeft)
        {
            for (int i = 0; i < 6; i++)
            {
                Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(), Main.rand.NextVector2Circular(1.5f, 1.5f), 0, new Color(149, 238, 238) * 0.5f, 0.55f);

                Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(), Main.rand.NextVector2Circular(1f, 1f), 0, new Color(209, 246, 239) * 0.5f, 0.5f);
            }
        }
