...
namespace BombusApisBee.Content.Dungeon.Items.SpectralBeeTome
{
    public class SpectralBee : CommonBeeProjectile
    {
        ...
        public override void OnKill(int timeLeft)
        {
            for (int i = 0; i < 5; i++)
            {
                Dust.NewDustPerfect(Projectile.Center + Main.rand.NextVector2Circular(5f, 5f), DustType<GlowFastDecelerate>(), Main.rand.NextVector2Circular(1.5f, 1.5f), 0, new Color(130, 153, 208), 0.35f);

                Dust.NewDustPerfect(Projectile.Center + Main.rand.NextVector2Circular(5f, 5f), DustType<GlowFastDecelerate>(), Main.rand.NextVector2Circular(2.5f, 2.5f), 0, new Color(189, 240, 224), 0.3f);
            }
        }
        ...
        public override bool SafePreDraw(ref Color lightColor)
        {
            ...
            Texture2D glowTex = Request<Texture2D>(Texture + (Giant ? "_Giant_Glowy" : "_Glowy")).Value;
