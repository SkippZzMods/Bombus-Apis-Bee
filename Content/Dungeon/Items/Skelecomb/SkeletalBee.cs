...
namespace BombusApisBee.Content.Dungeon.Items.Skelecomb
{
    public class SkeletalBee : CommonBeeProjectile
    {
        ...
        public override void OnKill(int timeLeft)
        {
            for (int i = 0; i < 5; i++)
            {
                Dust.NewDustPerfect(Projectile.Center + Main.rand.NextVector2Circular(5f, 5f), DustType<Dusts.GlowFastDecelerate>(), Main.rand.NextVector2Circular(1f, 1f), 0, new Color(100, 70, 107), 0.35f);

                Dust.NewDustPerfect(Projectile.Center + Main.rand.NextVector2Circular(5f, 5f), DustType<Dusts.GlowFastDecelerate>(), Main.rand.NextVector2Circular(1.5f, 1.5f), 0, new Color(238, 164, 255), 0.3f);
            }
        ...
        public override void SafeOnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            if (Main.rand.NextBool())
            {
                ...
                Projectile.NewProjectile(Projectile.GetSource_OnHit(target), pos, pos.DirectionTo(target.Center) * 25f, ProjectileType<SkeletalBeeSlash>(), (int)(Projectile.damage * 0.35f), 2.5f, Projectile.owner);
            }
        }

        public override void PostDraw(Color lightColor)
        {
            Texture2D tex = Request<Texture2D>("BombusApisBee/ExtraTextures/BeeGlowPREHM").Value;
            if (Giant)
                tex = Request<Texture2D>("BombusApisBee/ExtraTextures/BeeGlowGiant").Value;
    ...
    class SkeletalBeeSlash : BeeProjectile, IDrawPrimitive_
    {
        ...
        public override void AI()
        {
            ...
            Dust.NewDustPerfect(Projectile.Center, DustType<Dusts.GlowFastDecelerate>(), -Projectile.velocity.RotatedByRandom(0.35f) * 0.5f, 0, new Color(238, 164, 255), 0.35f);
        }
