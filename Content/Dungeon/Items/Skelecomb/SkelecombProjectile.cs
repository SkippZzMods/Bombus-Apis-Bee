    ...
    public class SkelecombProjectile : BeeProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (foundTarget && Vector2.Distance(Projectile.Center, targetCenter) > 20f)
                Projectile.velocity = (Projectile.velocity * 35f + (targetCenter - Projectile.Center).SafeNormalize(Vector2.UnitX) * 12f) / 36f;
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            for (int i = 0; i < 15; i++)
            {
                Dust.NewDustPerfect(Projectile.Center + Main.rand.NextVector2Circular(5f, 5f), DustType<Dusts.GlowFastDecelerate>(), Main.rand.NextVector2Circular(2f, 2f), 0, new Color(100, 70, 107), 0.45f);

                Dust.NewDustPerfect(Projectile.Center + Main.rand.NextVector2Circular(5f, 5f), DustType<Dusts.GlowFastDecelerate>(), Main.rand.NextVector2Circular(2.5f, 2.5f), 0, new Color(238, 164, 255), 0.55f);
            }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Texture2D texBloom = Request<Texture2D>("BombusApisBee/ExtraTextures/GlowAlpha").Value;
