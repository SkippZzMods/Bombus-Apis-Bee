...
namespace BombusApisBee.Content.Dungeon.Items.ManaInfusedHoneycomb
{
    public class ManaHoneycombHoldout : BeeProjectile
    {
        ...
        public override string Texture => "BombusApisBee/Content/Dungeon/Items/ManaInfusedHoneycomb/ManaInfusedHoneycomb";
        ...
        public override void AI()
        {
            ...
            if (Charge < MAXCHARGE)
            {
                if (Charge == MAXCHARGE - 1)
                {
                    ...
                    for (float k = 0; k < 6.28f; k += 0.1f)
                    {
                        ...
                        Dust.NewDustPerfect(starPos, DustType<Dusts.GlowFastDecelerate>(), new Vector2(x, y) * mult * 1.2f, 0, new Color(18, 18, 255), 0.35f);
                    }
                }
                Charge++;
            }
            ...
            if (!Owner.channel || modPlayer.BeeResourceCurrent <= 0)
            {
                if (Charge >= MAXCHARGE)
                {
                    if (Main.myPlayer == Projectile.owner)
                    {
                        ...
                        for (int i = 0; i < 5; i++)
                        {
                            Projectile.NewProjectile(Projectile.GetSource_FromAI(), tipPos, Projectile.velocity.RotatedByRandom(0.25f) * 6f, ProjectileType<ManaBee>(), Projectile.damage, 0, Owner.whoAmI);
                        }

                        for (int i = 0; i < 20; i++)
                        {
                            Dust.NewDustPerfect(tipPos, DustType<Dusts.GlowFastDecelerate>(), Projectile.velocity.RotatedByRandom(0.45f) * Main.rand.NextFloat(4f), 0, new Color(18, 18, 255), 0.45f);

                            Dust.NewDustPerfect(tipPos, DustType<Dusts.GlowFastDecelerate>(), Projectile.velocity.RotatedByRandom(0.45f) * Main.rand.NextFloat(4f), 0, new Color(100, 96, 255), 0.45f);

                            Dust.NewDustPerfect(tipPos, DustType<Dusts.GlowFastDecelerate>(), Main.rand.NextVector2Circular(2f, 2f), 0, new Color(18, 18, 255), 0.55f);

                            Dust.NewDustPerfect(tipPos, DustType<Dusts.GlowFastDecelerate>(), Main.rand.NextVector2Circular(2f, 2f), 0, new Color(100, 96, 255), 0.55f);
                        }

                        for (int i = 0; i < 12; i++)
                        {
                            Dust.NewDustPerfect(tipPos, DustType<Dusts.ManaStardust>(), Projectile.velocity.RotatedByRandom(0.45f) * Main.rand.NextFloat(4f), 0, default, Main.rand.NextFloat(0.5f, 1.3f));
                        }
                    }
            ...
            Dust.NewDustPerfect(pos, DustType<Dusts.GlowFastDecelerate>(), pos.DirectionTo(tipPos), 0, new Color(18, 18, 255), 0.35f);
            pos = tipPos + Main.rand.NextVector2CircularEdge(lerper, lerper);
            Dust.NewDustPerfect(pos, DustType<Dusts.GlowFastDecelerate>(), pos.DirectionTo(tipPos), 0, new Color(100, 96, 255), 0.35f);
        }
