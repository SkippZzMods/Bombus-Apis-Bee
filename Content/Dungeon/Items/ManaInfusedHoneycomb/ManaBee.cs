    ...
    public class ManaBee : BaseBeeProjectile
    {
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            for (float k = 0; k < 6.28f; k += 0.1f)
            {
                ...
                Dust.NewDustPerfect(Projectile.Center, DustType<Dusts.GlowFastDecelerate>(), new Vector2(x, y).RotatedBy(rot) * mult * 1.2f, 0, new Color(18, 18, 255), 0.45f);
            }

            if (Main.myPlayer == Projectile.owner)
                Projectile.NewProjectile(Projectile.GetSource_Death(), Projectile.Center, Main.rand.NextVector2CircularEdge(12f, 12f), ProjectileType<HomingManaStar>(), Projectile.damage / 2, 3.5f, Projectile.owner);
        }

        public override void SafeAI()
        {
            if (Main.rand.NextBool(3))
                Dust.NewDustPerfect(Projectile.Center + Projectile.velocity, DustType<Dusts.GlowFastDecelerate>(), Main.rand.NextVector2Circular(1.5f, 1.5f), 55,
    ...
    class HomingManaStar : BeeProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (foundTarget && Projectile.timeLeft < 400)
                Projectile.velocity = (Projectile.velocity * 35f + (targetCenter - Projectile.Center).SafeNormalize(Vector2.UnitX) * 14f) / 36f;
            else
            {
            ...
            Dust.NewDustPerfect(Projectile.Center + Projectile.rotation.ToRotationVector2() * 12f + Projectile.velocity, DustType<Dusts.Glow>(), Vector2.Zero, Scale: 0.35f, newColor: new Color(18, 18, 255));

            Dust.NewDustPerfect(Projectile.Center + Projectile.rotation.ToRotationVector2() * -12f + Projectile.velocity, DustType<Dusts.Glow>(), Vector2.Zero, Scale: 0.35f, newColor: new Color(100, 96, 255));

            if (Main.rand.NextBool(10))
                Dust.NewDustPerfect(Projectile.Center + Main.rand.NextVector2Circular(5f, 5f), DustType<Dusts.ManaStardust>(), Main.rand.NextVector2Circular(3.5f, 3.5f));
        }
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            for (int i = 0; i < 25; i++)
            {
                Dust.NewDustPerfect(Projectile.Center + Main.rand.NextVector2Circular(5f, 5f), DustType<Dusts.GlowFastDecelerate>(), Main.rand.NextVector2Circular(2.5f, 2.5f), 0, new Color(18, 18, 255), 0.45f);

                Dust.NewDustPerfect(Projectile.Center + Main.rand.NextVector2Circular(5f, 5f), DustType<Dusts.GlowFastDecelerate>(), Main.rand.NextVector2Circular(2.5f, 2.5f), 0, new Color(100, 96, 255), 0.45f);

                Dust.NewDustPerfect(Projectile.Center + Main.rand.NextVector2Circular(5f, 5f), DustType<Dusts.Glow>(), Main.rand.NextVector2Circular(2f, 2f), 0, new Color(100, 96, 255), 0.55f);
            }

            for (int i = 0; i < 15; i++)
            {
                Dust.NewDustPerfect(Projectile.Center + Main.rand.NextVector2Circular(5f, 5f), DustType<Dusts.ManaStardust>(), Main.rand.NextVector2Circular(3.5f, 3.5f), Scale: 1.25f);
            }
        }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Texture2D texGlow = Request<Texture2D>("BombusApisBee/Content/Space/Items/StarStrap/AstralStar_Glow").Value;
