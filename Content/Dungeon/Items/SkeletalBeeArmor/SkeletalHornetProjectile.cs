    ...
    public class SkeletalHornetProjectile : ModProjectile
    {
        ...
        private void EnragedAttacking(NPC target)
        {
            ...
            if (DashDelay <= 0)
            {
                ...
                for (int i = 0; i < 3; i++)
                {
                    ...
                    if (i == 2)
                        rotation = -0.1f;
                    Projectile.NewProjectileDirect(Projectile.GetSource_FromAI(), Projectile.Center, (Projectile.DirectionTo(targetCenter) * 15f).RotatedBy(rotation), ProjectileType<SkeletalStinger>(), (int)(Projectile.damage * 0.66f), 2f, player.whoAmI, 1f).tileCollide = false;
                }
                ...
                for (int i = 0; i < Repeats; ++i)
                {
                    ...
                    Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(), angle2.ToRotationVector2() * 3f, 25, new Color(200, 45, 20), 0.4f);
                }
            }
            else if (DashDelay <= 100)
            {
                ...
                if (Vector2.Distance(targetCenter, Projectile.Center) < 750f)
                    ...
                    {
                        SoundEngine.PlaySound(SoundID.Item17, Projectile.Center);
                        Projectile.NewProjectileDirect(Projectile.GetSource_FromAI(), Projectile.Center, Projectile.DirectionTo(targetCenter + target.velocity) * 21f, ProjectileType<SkeletalStinger>(), Projectile.damage, 2f, player.whoAmI, 1f).tileCollide = false;
                        StingerDelay = 25f;
                    }
            }
        }
        ...
        private void NormalAttacking(NPC target)
        {
            ...
            if (Vector2.Distance(targetCenter, Projectile.Center) < 550f && Collision.CanHitLine(Projectile.Center, 1, 1, targetCenter, 1, 1))
                ...
                {
                    SoundEngine.PlaySound(SoundID.Item17, Projectile.Center);
                    Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center, Projectile.DirectionTo(targetCenter) * 15f, ProjectileType<SkeletalStinger>(), Projectile.damage, 2f, player.whoAmI);
                    StingerDelay = 35f;
                }
