...
namespace BombusApisBee.Content.Crimson.Items.Cystcomb
{
    ...
    {
        public override string Texture => "BombusApisBee/Content/Crimson/Items/Cystcomb/CystComb";
        ...
        {
            return BeeUtils.PiecewiseAnimation((float)throwTimer / MaxThrowTimer, new BeeUtils.CurveSegment[]
            {
                pullback,
                throwout
            });
        ...
        public override void AI()
        {
            ...
            else
            {
                ...
                Dust.NewDustPerfect(Projectile.Center + Projectile.rotation.ToRotationVector2() * 10f + Projectile.velocity, DustType<Dusts.Glow>(), Vector2.Zero, Scale: 0.5f, newColor: new Color(196, 102, 9));

                Dust.NewDustPerfect(Projectile.Center + Projectile.rotation.ToRotationVector2() * -10f + Projectile.velocity, DustType<Dusts.Glow>(), Vector2.Zero, Scale: 0.5f, newColor: new Color(253, 152, 0));
            }
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            Projectile.NewProjectile(Projectile.GetSource_OnHit(target), target.Center, Vector2.Zero, ProjectileType<IchorExplosion>(), (int)(Projectile.damage * 1.5f), 0f, Projectile.owner, 55);

            for (int i = 0; i < 3; i++)
            {
                Projectile.NewProjectile(Projectile.GetSource_OnHit(target), target.Center, Main.rand.NextVector2Circular(3f, 3f), ProjectileType<IchorBee>(), Projectile.damage / 2, 0f, Projectile.owner);
            }

            for (int i = 0; i < 25; i++)
            {
                Dust.NewDustPerfect(target.Center + Main.rand.NextVector2Circular(5f, 5f), DustType<Dusts.GlowFastDecelerate>(), Main.rand.NextVector2Circular(4f, 4f), 0, new Color(253, 152, 0), 0.85f);

                Dust.NewDustPerfect(target.Center + Main.rand.NextVector2Circular(5f, 5f), DustType<Dusts.GlowFastDecelerate>(), Main.rand.NextVector2Circular(4.5f, 4.5f), 0, new Color(196, 102, 9), 0.7f);
            }
        }
    ...
    class IchorExplosion : ModProjectile, IDrawPrimitive_
    {
        ...
        public override void AI()
        {
            ...
            for (int k = 0; k < 6; k++)
            {
                ...
                Dust.NewDustPerfect(Projectile.Center + Vector2.One.RotatedBy(rot) * Radius, DustType<Dusts.GlowFastDecelerate>(),
