...
namespace BombusApisBee.Content.Jungle.Items.Needleshot
{
    public class Needleshot : BeekeeperWeapon
    {
        ...
        public override void SafeSetDefaults()
        {
            ...
            Item.shoot = ProjectileType<HighVelocityStinger>();
            Item.shootSpeed = 25f;
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            for (int i = 0; i < 7; i++)
            {
                Dust.NewDustPerfect(position, DustType<Dusts.StingerDust>(), velocity.RotatedByRandom(0.35f) * Main.rand.NextFloat(0.25f), Main.rand.Next(50, 125)).noGravity = true;
            ...
            if (++shot >= 5)
            {
                ...
                Projectile.NewProjectile(source, pos, velocity * 0.75f, ProjectileType<Hivebomb>(), (int)(damage * 0.65f), knockback, player.whoAmI);
                for (int i = 0; i < 20; i++)
                {
                    Dust.NewDustPerfect(pos, DustType<Dusts.HoneyDust>(), velocity.RotatedByRandom(0.35f) * Main.rand.NextFloat(0.25f), Main.rand.Next(50, 125)).noGravity = true;
