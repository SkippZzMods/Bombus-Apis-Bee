...
namespace BombusApisBee.Content.Jungle.Items.HiveBandAccessory
{
    ...
    {
        public override bool Active => Main.projectile.Any(x => x.active && x.type == ProjectileType<BandHoneyGlob>()) || Main.dust.Any(x => x.active && x.type == DustType<BandHoneyDust>());
        ...
        public override void DrawShapes(SpriteBatch spriteBatch)
        {
            ...
            var honey = Main.projectile.Where(n => n.active && n.type == ProjectileType<BandHoneyGlob>());
            foreach (Projectile proj in honey)
            {
                (proj.ModProjectile as BandHoneyGlob).DrawMetaball();
            }

            var honeyDust = Main.dust.Where(n => n.active && n.type == DustType<BandHoneyDust>());
            foreach (Dust dust in honeyDust)
            {
                borderNoise.Parameters["offset"].SetValue((float)Main.time / 1000f + dust.rotation);
                spriteBatch.Draw(Request<Texture2D>("BombusApisBee/Content/Jungle/Items/HiveBandAccessory/BandHoneyDust").Value, (dust.position - Main.screenPosition) / 2, null, Color.White, 0f, Vector2.One * 256f, dust.scale / 64f, SpriteEffects.None, 0);
            }
        ...
        public override bool PostDraw(SpriteBatch spriteBatch, Texture2D target)
        {
            ...
            effect.Parameters["distort"].SetValue(Request<Texture2D>("BombusApisBee/ExtraTextures/SwirlyNoiseLooping").Value);
