using Terraria;
namespace BombusApisBee.Content.Jungle.Items.HiveBandAccessory
{
    ...
    class HiveBandPlayer : ModPlayer
    {
        ...
        {
            if (!equipped || proj.type == ProjectileType<BandHoneyGlob>() || Player.HasBuff<BandOfTheHiveCooldown>())
                return;
        ...
        void Explode(NPC target)
        {
            ...
            for (int i = 0; i < 50; i++)
            {
                Projectile.NewProjectile(Player.GetSource_OnHit(target), target.Center, Main.rand.NextVector2Circular(15f, 15f), ProjectileType<BandHoneyGlob>(), (int)(damage * 0.5f), 0f, Player.whoAmI);
            }

            for (int i = 0; i < 35; i++)
            {
                float angle = 6.2831855f * i / 35f;
                Projectile.NewProjectile(Player.GetSource_OnHit(target), target.Center, angle.ToRotationVector2() * 7.5f, ProjectileType<BandHoneyGlob>(), (int)(damage * 0.5f), 0f, Player.whoAmI);
            }

            for (int i = 0; i < 200; i++)
            {
                Dust.NewDustPerfect(target.Center, DustType<BandHoneyDust>(), Main.rand.NextVector2Circular(10f, 10f), 0, default, 5f);
            }

            Projectile.NewProjectile(Player.GetSource_OnHit(target), target.Center, Vector2.Zero, ProjectileType<BandHoneyExplosion>(), damage, 0, Player.whoAmI, 150, 55);
    ...
    class BandHoneyGlob : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (tile.HasTile && Main.tileSolid[tile.TileType] && !TileID.Sets.Platforms[tile.TileType])
            {
                ...
                {
                    Dust dust = Dust.NewDustPerfect(Projectile.Center + Main.rand.NextVector2Circular(rad, rad), DustType<BandHoneyDust>(), Vector2.UnitY.RotatedByRandom(1f) * Main.rand.NextFloat(0.1f, 0.5f), 0, default, 2 * Projectile.scale);
    ...
    class BandHoneyExplosion : ModProjectile, IDrawPrimitive_
    {
        ...
        public override void AI()
        {
            ...
            for (int k = 0; k < 6; k++)
            {
                ...
                Dust.NewDustPerfect(Projectile.Center + Vector2.One.RotatedBy(rot) * (Radius * 0.75f), DustType<GlowFastDecelerate>(),
