...
namespace BombusApisBee.Content.Jungle.Items.TriTipStinger
{
    ...
    class TriTipStingerNPC : GlobalNPC
    {
                        ...
                        for (int i = 0; i < Main.maxNPCs; i++)
                        {
                            ...
                            if (nPC.active && nPC != npc && !nPC.GetGlobalNPC<TriTipStingerNPC>().inflicted && npc.Distance(nPC.Center) < 200f)
                                if (Main.rand.NextBool(15))
                                {
                                    ...
                                    nPC.GetGlobalNPC<TriTipStingerNPC>().lastPlayerInflictedWhoAmI = lastPlayerInflictedWhoAmI;
                                }
                        }
                    }
        ...
        void doInflictedEffects(NPC npc)
        {
            ...
            {
                Dust.NewDustPerfect(npc.Center + Main.rand.NextVector2CircularEdge(npc.width / 2, npc.height / 2), DustType<TriTipStingerDust>(), Vector2.Zero, 0, default, Main.rand.NextFloat(scale, scale * 1.5f)).noGravity = true;
                Dust.NewDustPerfect(npc.Center + Main.rand.NextVector2Circular(npc.width / 2, npc.height / 2), DustType<TriTipStingerDust>(), Vector2.Zero, 0, default, Main.rand.NextFloat(scale, scale * 1.5f)).noGravity = true;
            }

            if (Main.rand.NextBool(15))
                Dust.NewDustPerfect(npc.Center + Main.rand.NextVector2Circular(npc.width / 2, npc.height / 2), DustType<TriTipStingerDust>(), Vector2.UnitY * Main.rand.NextFloat(), 0, default, Main.rand.NextFloat(scale * 0.5f, scale * 2f));
        }
    }
}
