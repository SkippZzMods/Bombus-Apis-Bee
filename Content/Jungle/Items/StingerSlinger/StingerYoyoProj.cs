...
namespace BombusApisBee.Content.Jungle.Items.StingerSlinger
{
    public class StingerYoyoProj : BeeProjectile
    {
        ...
        public override void AI()
        {
            ...
            {
                Dust dust = Dust.NewDustDirect(Projectile.position, Projectile.width, Projectile.height, DustType<StingerDust>(), 0f, 0f, 65);
                dust.noGravity = true;
                dust.scale = 0.9f;
                dust.velocity *= 0.5f;
            }
                ...
                if (target != null)
                {
                    if (StingerTimer > 30 && Collision.CanHitLine(Projectile.Center, 1, 1, target.Center, 1, 1))
                    {
                        ...
                        else
                        {
                            ...
                            Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center, Projectile.DirectionTo(target.Center) * 14.5f, ProjectileType<StingerFriendly>(), (int)(Projectile.damage * 0.6f), 2.5f, Projectile.owner);

                            for (int i = 0; i < 7; i++)
                            {
                                Vector2 velocity = Projectile.DirectionTo(target.Center) * 14.5f;
                                Dust.NewDustPerfect(Projectile.Center, DustType<StingerDust>(), velocity.RotatedByRandom(0.35f) * Main.rand.NextFloat(0.25f), Main.rand.Next(50, 125)).noGravity = true;
            ...
            else if (StingerTimer > 45)
            {
                ...
                else
                {
                    ...
                    for (int i = 0; i < 4; i++)
                    {
                        Projectile.NewProjectileDirect(Projectile.GetSource_FromAI(), Projectile.Center, Vector2.One.RotatedBy(MathHelper.PiOver2 * i) * 10f, ProjectileType<HomingStinger>(), Projectile.damage * 2 / 3, 2f, Projectile.owner);
                    }
                    ...
                    for (int i = 0; i < 15; i++)
                    {
                        Dust.NewDustPerfect(Projectile.Center, DustType<StingerDust>(), Main.rand.NextVector2Circular(3.5f, 3.5f), Main.rand.Next(50, 150), Scale: 1.25f).noGravity = true;
