namespace BombusApisBee.Content.Jungle.Items.ChlorophyteHoneycomb
{
    ...
    {
        public override string Texture => "BombusApisBee/Content/Jungle/Items/ChlorophyteHoneycomb/ChlorophyteHoneycomb";
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            if (Main.myPlayer == Projectile.owner)
            {
                ...
                {
                    Projectile.NewProjectile(Projectile.GetSource_Death(), Projectile.Center, Projectile.velocity.RotatedByRandom(0.6f) * Main.rand.NextFloat(0.25f, 1f), ProjectileType<Chlorospore>(), (int)(Projectile.damage * 0.66f), 0f, Projectile.owner);
                }

                for (int i = 0; i < Main.rand.Next(3, 6); i++)
                {
                    Projectile.NewProjectile(Projectile.GetSource_Death(), Projectile.Center, Projectile.velocity.RotatedByRandom(0.25f) * Main.rand.NextFloat(0.75f, 1f), ProjectileType<Chloroshard>(), Projectile.damage / 2, 2.5f, Projectile.owner);
                }

                for (int i = 0; i < Main.rand.Next(4, 7); i++)
                {
                    Projectile.NewProjectile(Projectile.GetSource_Death(), Projectile.Center, Projectile.velocity.RotatedByRandom(0.3f) * 0.25f, ProjectileType<ChlorophyteBee>(), Projectile.damage / 2, 0, Projectile.owner);
                }

                for (int i = 0; i < 30; i++)
                {
                    Dust.NewDustPerfect(Projectile.Center, DustType<Dusts.GlowFastDecelerate>(), Projectile.velocity.RotatedByRandom(0.4f) * Main.rand.NextFloat(), 0, new Color(117, 216, 19), 1f);

                    Dust.NewDustPerfect(Projectile.Center, DustType<Dusts.GlowFastDecelerate>(), Projectile.velocity.RotatedByRandom(0.4f) * Main.rand.NextFloat(), 0, new Color(36, 97, 51), 1f);

                    Dust.NewDustPerfect(Projectile.Center + Main.rand.NextVector2Circular(15f, 15f), DustType<Dusts.GlowFastDecelerate>(), Main.rand.NextVector2Circular(4f, 4f), 0, new Color(117, 216, 19), 0.75f);

                    Dust.NewDustPerfect(Projectile.Center + Main.rand.NextVector2Circular(15f, 15f), DustType<Dusts.GlowFastDecelerate>(), Main.rand.NextVector2Circular(4f, 4f), 0, new Color(36, 97, 51), 0.75f);
                }

                for (int i = 0; i < 45; ++i)
                {
                    float angle2 = 6.2831855f * i / 45;
                    Dust.NewDustPerfect(Projectile.Center, DustType<Dusts.GlowFastDecelerate>(), angle2.ToRotationVector2() * 2.75f, 25, new Color(117, 216, 19), 0.65f);
                }
            }
        }
    ...
    public class Chloroshard : BeeProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (foundTarget && Projectile.timeLeft < 460)
                Projectile.velocity = (Projectile.velocity * 40f + (targetCenter - Projectile.Center).SafeNormalize(Vector2.UnitX) * 16f) / 41f;
            else
            {
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                Dust.NewDustPerfect(Projectile.Center, DustType<Dusts.GlowFastDecelerate>(), Main.rand.NextVector2Circular(1.5f, 1.5f), 0, new Color(117, 216, 19), 0.35F);
            }
        }
    ...
    public class Chlorospore : BeeProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Main.rand.NextBool(10))
                Dust.NewDustDirect(Projectile.position, Projectile.width, Projectile.height, DustType<Dusts.Glow>(), 0, 0, 0, new Color(214, 158, 79), MathHelper.Lerp(1f, 0f, Projectile.alpha / 255f));
