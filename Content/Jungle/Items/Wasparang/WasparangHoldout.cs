...
namespace BombusApisBee.Content.Jungle.Items.Wasparang
{
    public class WasparangHoldout : ModProjectile
    {
        ...
        {
            return BeeUtils.PiecewiseAnimation(chargeTimer / (float)maxCharge, new BeeUtils.CurveSegment[]
            {
                pullback
            });
        }
            ...
            else
            {
                ...
                else
                {
                    if (Main.myPlayer == owner.whoAmI && boomerangTimer == 0)
                    {
                        ...
                        Projectile.velocity = Projectile.DirectionTo(Main.MouseWorld) * MathHelper.Lerp(20f, 28.5f, chargeTimer / (float)maxCharge);
                    ...
                    if (boomerangTimer > 45f)
                    {
                        ...
                        float num = owner.position.X + owner.width / 2 - pos.X;
                        float num2 = owner.position.Y + owner.height / 2 - pos.Y;
                    ...
                    if (++Projectile.ai[1] % 15 == 0)
                        Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center, Vector2.One.RotatedByRandom(6.28f) * 3f, ProjectileID.Wasp, (int)(Projectile.damage * MathHelper.Lerp(0.66f, 1.5f, chargeTimer / (float)maxCharge)), 1f, Projectile.owner);

                    Dust.NewDustPerfect(Projectile.Center + Projectile.rotation.ToRotationVector2() * 20f, DustType<WaspDustBlack>(), Vector2.Zero, Scale: 1.2f).noGravity = true;

                    Dust.NewDustPerfect(Projectile.Center + Projectile.rotation.ToRotationVector2() * -20f, DustType<WaspDustOrange>(), Vector2.Zero, Scale: 1.2f).noGravity = true;
                }
            }
        }
        ...
        {
            modifiers.SourceDamage *= MathHelper.Lerp(1f, 3.5f, chargeTimer / (float)maxCharge);
            modifiers.Knockback *= MathHelper.Lerp(1f, 2f, chargeTimer / (float)maxCharge);
        }
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            if (numHits < 3)
            {
                ...
                {
                    Projectile.NewProjectileDirect(Projectile.GetSource_OnHit(target), Projectile.Center, Vector2.UnitY.RotatedByRandom(0.4f) * Main.rand.NextFloat(-8f, -15f), ProjectileType<HomingStinger>(),
                        (int)(Projectile.damage * MathHelper.Lerp(0.66f, 1.1f, chargeTimer / (float)maxCharge)), 2.5f, owner.whoAmI);
                }
            }
            ...
            for (int i = 0; i < Repeats; ++i)
            {
                ...
                Dust dust3 = Dust.NewDustPerfect(target.Center, DustType<WaspDustOrange>(), null, 0, default, 1.1f);
                dust3.velocity = angle2.ToRotationVector2() * 1.25f;

                Dust.NewDustPerfect(target.Center, DustType<WaspDustBlack>(), angle2.ToRotationVector2() * 1.75f, 0, default, 1.1f);

                Dust.NewDustPerfect(target.Center, DustType<WaspDustOrange>(), angle2.ToRotationVector2() * 2.25f, 0, default, 1.1f);
            }
        }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Texture2D textureWhite = Request<Texture2D>(Texture + "_White").Value;
