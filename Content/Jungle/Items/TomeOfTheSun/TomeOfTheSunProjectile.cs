...
namespace BombusApisBee.Content.Jungle.Items.TomeOfTheSun
{
    public class TomeOfTheSunProjectile : BeeProjectile
    {
        ...
        public override void AI()
        {
            for (int i = 0; i < 2; i++)
            {
                ...
                Dust.NewDustPerfect(pos, DustType<GlowFastDecelerate>(), pos.DirectionTo(Projectile.Center) * 4.25f, 0, new Color(245, 245, 149), 0.35f);
            }
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            for (int i = 0; i < 30; i++)
            {
                Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(), Main.rand.NextVector2Circular(4.5f, 4.5f), 0, new Color(245, 245, 149), 0.85f);

                Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(), Main.rand.NextVector2Circular(4f, 4f), 0, new Color(222, 173, 40), 0.9f);
            }
            new SoundStyle("BombusApisBee/Sounds/Item/FireHit").PlayWith(Projectile.Center);

            for (int i = 0; i < Main.rand.Next(2, 4); i++)
            {
                Projectile.NewProjectile(Projectile.GetSource_Death(), Projectile.Center, Main.rand.NextVector2CircularEdge(18f, 18f) * Main.rand.NextFloat(0.5f, 1f), ProjectileType<TomeOfTheSunProjectileHoming>(), Projectile.damage, 3.5f, Projectile.owner);
            }
        }
        ...
        {
            Texture2D tex = Request<Texture2D>("BombusApisBee/ExtraTextures/BloomFlare").Value;
    ...
    class TomeOfTheSunProjectileHoming : BeeProjectile, IDrawPrimitive_
    {
        ...
        public override void AI()
        {
            ...
            Dust.NewDustPerfect(Projectile.Center - Projectile.velocity + Main.rand.NextVector2Circular(15f, 15f), DustType<GlowFastDecelerate>(), Vector2.Zero, 0, new Color(222, 173, 40), 0.5f);
            ...
            {
                Projectile.velocity = (Projectile.velocity * 25f + (targetCenter - Projectile.Center).SafeNormalize(Vector2.UnitX) * 35f) / 26f;
            }
        }
        ...
        public override void OnKill(int timeLeft)
        {
            for (int i = 0; i < 10; i++)
            {
                Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(), Main.rand.NextVector2Circular(2.5f, 2.5f), 0, new Color(245, 245, 149), 0.55f);

                Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(), Main.rand.NextVector2Circular(2f, 2f), 0, new Color(222, 173, 40), 0.5f);
            }
        }
