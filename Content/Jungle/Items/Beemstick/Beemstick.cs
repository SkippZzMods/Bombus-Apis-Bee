...
namespace BombusApisBee.Content.Jungle.Items.Beemstick
{
    public class Beemstick : BeeDamageItem
    {
        ...
        public override void SafeSetDefaults()
        {
            ...
            Item.shootSpeed = 6;
            Item.UseSound = new SoundStyle("BombusApisBee/Sounds/Item/HeavyShotgun") with { Volume = 0.6f, PitchVariance = 0.1f };
            Item.scale = 1;
            honeyCost = 5;
        }
        ...
        public override void UseStyle(Player player, Rectangle heldItemFrame)
        {
            ...
            if (animProgress < 0.55f)
            {
                ...
                itemPosition += itemRotation.ToRotationVector2() * MathHelper.Lerp(0f, 7f, EaseFunction.EaseCircularInOut.Ease(lerper));
            }
            else
            {
                if (animProgress < 0.75f)
                {
                    ...
                    itemPosition += itemRotation.ToRotationVector2() * MathHelper.Lerp(7f, -3f, EaseFunction.EaseQuinticInOut.Ease(lerper));
                }
                else
                {
                    float lerper = (animProgress - 0.75f) / 0.25f;
                    itemPosition += itemRotation.ToRotationVector2() * MathHelper.Lerp(-3f, 5f, EaseFunction.EaseQuinticInOut.Ease(lerper));
                }
            }
            ...
            if (Main.rand.NextBool(20))
                Dust.NewDustPerfect(itemPosition + new Vector2(32.5f, -5f * player.direction).RotatedBy(player.compositeFrontArm.rotation + 1.5707964f * player.gravDir), DustType<SmokeDustAlt>(), Vector2.UnitY * -2f, 100, default, 0.03f).noGravity = true;
        ...
        public override void UseItemFrame(Player player)
        {
            ...
            if (animProgress < 0.55f)
            {
                if (animProgress < 0.1f)
                {
                    ...
                    rotation += MathHelper.Lerp(0f, -0.5f, EaseFunction.EaseCircularOut.Ease(lerper)) * player.direction;
                }
                else
                {
                    float lerper = (animProgress - 0.1f) / 0.45f;
                    rotation += MathHelper.Lerp(-0.5f, 0, EaseFunction.EaseCircularInOut.Ease(lerper)) * player.direction;
                }
            }
            else
            {
                ...
                {
                    float lerper = (animProgress - 0.55f) / 0.2f;
                    rotation += MathHelper.Lerp(0f, 0.15f, EaseFunction.EaseCircularInOut.Ease(lerper)) * player.direction;
                ...
                {
                    float lerper = (animProgress - 0.75f) / 0.25f;
                    rotation += MathHelper.Lerp(0.15f, 0.05f, EaseFunction.EaseCircularInOut.Ease(lerper)) * player.direction;
                }
            }
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            {
                player.SpawnBee(source, position, velocity.RotatedByRandom(15f.AsRadians()), damage, knockback);
            }
            for (int i = 0; i < Main.rand.Next(2, 4); i++)
            {
                Projectile.NewProjectileDirect(source, position, velocity.RotatedByRandom(10f.AsRadians()), ProjectileType<BeeBuckshot>(), damage / 2, knockback * 3, player.whoAmI);
            }

            for (int i = 0; i < 10; i++)
            {
                Dust.NewDustPerfect(position, DustType<Glow>(), velocity.RotatedByRandom(0.3f) * Main.rand.NextFloat(1f, 2f), 0, new Color(255, 220, 50, 0), 0.15f);
            ...
            Dust dust = Dust.NewDustPerfect(position, DustType<SmokeDustAlt>(), Main.rand.NextVector2Circular(1f, 1f) + -velocity * 0.35f + Vector2.UnitY * -1.5f, 0, default, 0.1f);
            dust.rotation = Main.rand.NextFloat(6.28f);

            dust = Dust.NewDustPerfect(position, DustType<SmokeDustAlt>(), Vector2.UnitY * -1.5f, 0, default, 0.1f);
            dust.rotation = Main.rand.NextFloat(6.28f);

            dust = Dust.NewDustPerfect(position, DustType<BeemstickMuzzleFlashDust>(), Vector2.Zero, 0, default, 1f);
    ...
    class BeemstickMuzzleFlashDust : ModDust
    {
        ...
        public override bool PreDraw(Dust dust)
        {
            ...
            Texture2D tex = Request<Texture2D>("BombusApisBee/ExtraTextures/HoneyShotgunMuzzleFlash").Value;
            Texture2D texBlur = Request<Texture2D>("BombusApisBee/ExtraTextures/HoneyShotgunMuzzleFlash_Blur").Value;
            Texture2D texGlow = Request<Texture2D>("BombusApisBee/ExtraTextures/HoneyShotgunMuzzleFlash_Glow").Value;
