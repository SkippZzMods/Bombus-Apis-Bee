...
namespace BombusApisBee.Content.Jungle.Items.BladeOfAculeus
{
    ...
    {
        public override string Texture => "BombusApisBee/Content/Jungle/Items/BladeOfAculeus/BladeOfAculeus";
        ...
        private void UpSlash()
        {
            ...
            if (progress < 0.5f)
            {
                ...
                Projectile.rotation = Projectile.velocity.ToRotation() + MathHelper.PiOver4 + MathHelper.Lerp(-2f, 0f, EaseFunction.EaseBackIn.Ease(lerper)) * originalDirection;

                Vector2 offset = Vector2.Lerp(new Vector2(0), new Vector2(20, -10 * originalDirection), EaseFunction.EaseBackIn.Ease(lerper)).RotatedBy(Projectile.rotation - MathHelper.PiOver4);

                Projectile.Center = owner.MountedCenter + (Projectile.rotation - MathHelper.PiOver4).ToRotationVector2() * MathHelper.Lerp(35f, 30f, EaseFunction.EaseBackIn.Ease(lerper)) + offset;
                ...
                float armRot = Projectile.velocity.ToRotation() - MathHelper.PiOver2 + MathHelper.Lerp(-1.8f, 0f, EaseFunction.EaseBackIn.Ease(lerper)) * originalDirection;

                owner.SetCompositeArmFront(true, Player.CompositeArmStretchAmount.Full, armRot);

                Projectile.scale = MathHelper.Lerp(0.8f, 1f, EaseFunction.EaseBackIn.Ease(lerper));
            ...
            else
            {
                if (!swung)
                {
                    ...
                    for (int i = -1; i < 2; i++)
                    {
                        Projectile.NewProjectile(Projectile.GetSource_FromAI(), owner.Center, Projectile.velocity.RotatedBy(i * 0.05f) * 5f, ProjectileType<AculeusBladeStinger>(), Projectile.damage, Projectile.knockBack, Projectile.owner);
                    }
                }

                float lerper = 1f - Projectile.timeLeft / (float)Math.Floor(maxTimeLeft - maxTimeLeft * 0.5f);

                Projectile.rotation = Projectile.velocity.ToRotation() + MathHelper.PiOver4 + MathHelper.Lerp(0f, 2f, EaseFunction.EaseQuinticOut.Ease(lerper)) * originalDirection;

                Vector2 offset = Vector2.Lerp(new Vector2(20, -10 * originalDirection), new Vector2(5, 15 * originalDirection), EaseFunction.EaseQuinticOut.Ease(lerper)).RotatedBy(Projectile.rotation - MathHelper.PiOver4);

                Projectile.Center = owner.MountedCenter + (Projectile.rotation - MathHelper.PiOver4).ToRotationVector2() * MathHelper.Lerp(30f, 35f, EaseFunction.EaseQuinticOut.Ease(lerper)) + offset;

                tipPosition = owner.MountedCenter + offset + (Projectile.rotation - MathHelper.PiOver4 - 0.1f * originalDirection).ToRotationVector2() * 60f;

                float armRot = Projectile.velocity.ToRotation() - MathHelper.PiOver2 + MathHelper.Lerp(0f, 2.2f, EaseFunction.EaseQuinticOut.Ease(lerper)) * originalDirection;

                owner.SetCompositeArmFront(true, Player.CompositeArmStretchAmount.Full, armRot);

                Projectile.scale = 0.8f + (float)Math.Sin(EaseFunction.EaseQuinticOut.Ease(lerper) * 3.1415927f) * 0.75f * 0.75f;
            }
        }
        ...
        private void DownSlash()
        {
            ...
            if (progress < 0.5f)
            {
                ...
                Vector2 offset = Vector2.Lerp(new Vector2(5, 15 * originalDirection), new Vector2(20, -10 * originalDirection), EaseFunction.EaseBackIn.Ease(lerper)).RotatedBy(Projectile.rotation - MathHelper.PiOver4);

                Projectile.Center = owner.MountedCenter + (Projectile.rotation - MathHelper.PiOver4).ToRotationVector2() * MathHelper.Lerp(35f, 30f, EaseFunction.EaseBackIn.Ease(lerper)) + offset;

                tipPosition = owner.MountedCenter + offset + (Projectile.rotation - MathHelper.PiOver4).ToRotationVector2() * 60f;

                float armRot = Projectile.velocity.ToRotation() - MathHelper.PiOver2 + MathHelper.Lerp(2.2f, 0f, EaseFunction.EaseBackIn.Ease(lerper)) * originalDirection;
            ...
            else
            {
                ...
                Vector2 offset = Vector2.Lerp(new Vector2(20, -10 * originalDirection), new Vector2(0), EaseFunction.EaseQuinticOut.Ease(lerper)).RotatedBy(Projectile.rotation - MathHelper.PiOver4);

                Projectile.Center = owner.MountedCenter + (Projectile.rotation - MathHelper.PiOver4).ToRotationVector2() * MathHelper.Lerp(30f, 35f, EaseFunction.EaseQuinticOut.Ease(lerper)) + offset;

                tipPosition = owner.MountedCenter + offset + (Projectile.rotation - MathHelper.PiOver4).ToRotationVector2() * 60f;

                float armRot = Projectile.velocity.ToRotation() - MathHelper.PiOver2 + MathHelper.Lerp(0f, -1.8f, EaseFunction.EaseQuinticOut.Ease(lerper)) * originalDirection;
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            for (int i = 0; i < 15; i++)
            {
                Dust.NewDustPerfect(target.Center + target.DirectionTo(owner.Center) * (target.width / 2), DustType<StingerDust>(), target.DirectionTo(owner.Center).RotatedByRandom(0.6f) * Main.rand.NextFloat(0f, 10f), Main.rand.Next(50, 255), default, 1.5f).noGravity = true;
                ...
                Dust.NewDustPerfect(target.Center + target.DirectionTo(owner.Center) * (target.width / 2), DustType<StingerDust>(), target.DirectionTo(owner.Center).RotatedByRandom(1.5f) * Main.rand.NextFloat(0f, 10f), Main.rand.Next(50, 255), default, 1.5f).noGravity = true;
        ...
        {
            DrawPrimitives();
            Texture2D sword = Request<Texture2D>(Texture + (flipBlade ? "_Flipped" : "")).Value;
            ...
            if (drawAfterImages)
            {
                for (int i = 15; i > 0; i--)
                {
                    ...
                    {
                        Texture2D swordBlade = Request<Texture2D>(Texture + "_Blade" + (oldFlipBlade[i] ? "_Flipped" : "")).Value;
        ...
        public void DrawPrimitives()
        {
            ...
            if (drawAfterImages)
            {
                ...
                effect.Parameters["sampleTexture"].SetValue(Request<Texture2D>("BombusApisBee/ShaderTextures/ShadowTrail").Value);
