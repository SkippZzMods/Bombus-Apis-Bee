...
namespace BombusApisBee.Content.Jungle.Items.NectarBolt
{
    public class NectarBolt : BeekeeperWeapon
    {
        ...
        public override void SafeSetDefaults()
        {
            ...
            Item.autoReuse = true;
            Item.shoot = ProjectileType<NectarBoltProjectile>();
            Item.shootSpeed = 9f;
            honeyCost = 4;
        }
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            for (int i = 0; i < 10; i++)
            {
                Dust.NewDustPerfect(position + Vector2.Normalize(velocity) * 20f, DustType<Dusts.GlowFastDecelerate>(), velocity.RotatedByRandom(0.3f) * Main.rand.NextFloat(0f, 0.5f), 0, new Color(255, 255, 150), 0.65f);
            }
        ...
        public override void AddRecipes()
        {
            ...
            modRecipe.AddIngredient(ItemType<PollenItem>(), 15);
