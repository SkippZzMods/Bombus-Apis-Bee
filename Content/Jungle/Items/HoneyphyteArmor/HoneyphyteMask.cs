...
namespace BombusApisBee.Content.Jungle.Items.HoneyphyteArmor
{
    ...
    public class HoneyphyteMask : BeeKeeperItem
    {
        ...
        {
            CreateRecipe(1).AddIngredient(ItemID.ChlorophyteBar, 12).AddIngredient(ItemID.Ectoplasm, 3).AddIngredient(ItemID.HoneyBlock, 10).AddIngredient(ItemID.BottledHoney, 6).AddIngredient(ModContent.ItemType<PollenItem>(), 30).AddTile(TileID.MythrilAnvil).Register();
        }
    }
        ...
        public override void AI()
        {
            ...
            else if (Charge >= BombusApisBeePlayer.HONEY_LASER_CHARGE_MAX && StartupTimer <= 0)
            {
                if (Main.rand.NextBool(10))
                    Dust.NewDustPerfect(Projectile.Center + Main.rand.NextVector2Circular(30f, 30f), ModContent.DustType<GlowFastDecelerate>(), Vector2.UnitY * -Main.rand.NextFloat(2f, 3f), 0, new Color(200, 100, 20), 0.5f);
            }
            ...
            if (!pulsed && Charge >= BombusApisBeePlayer.HONEY_LASER_CHARGE_MAX)
            {
                ...
                for (int i = 0; i < 15; i++)
                {
                    Dust.NewDustPerfect(Projectile.Center, ModContent.DustType<Glow>(), Main.rand.NextVector2Circular(5f, 5f), 0, new Color(200, 100, 20), 1f);
                }
            }
            ...
            if (StartupTimer > 0)
            {
                ...
                if (StartupTimer == 1)
                {
                    ...
                    for (int i = 0; i < 30; i++)
                    {
                        Dust.NewDustPerfect(Projectile.Center, ModContent.DustType<GlowFastDecelerate>(), Main.rand.NextVector2Circular(5f, 5f), 0, new Color(200, 100, 20), 0.5f);
