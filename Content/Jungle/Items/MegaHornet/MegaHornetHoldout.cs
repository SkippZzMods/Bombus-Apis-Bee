        ...
        public void ShootThings(Vector2 barrelPos)
        {
            if (owner.UseBeeResource((Main.player[Projectile.owner].HeldItem.ModItem as BeeDamageItem).honeyCost + 1))
            {
                ...
                if (Main.myPlayer == Projectile.owner)
                {
                    ...
                    Projectile.NewProjectile(Projectile.GetSource_FromThis(), barrelPos, Projectile.velocity.RotatedByRandom(0.15f) * 17.5f, ProjectileType<StingerFriendly>(), damage, Projectile.knockBack, Projectile.owner);
                    if (Main.rand.NextBool())
                        Projectile.NewProjectile(Projectile.GetSource_FromThis(), barrelPos, Projectile.velocity.RotatedByRandom(0.15f) * 15f, ProjectileType<HomingStinger>(), damage, Projectile.knockBack, Projectile.owner);
                }
