namespace BombusApisBee.Content.Projectiles
{
    class NectarHealingBolt : BeeProjectile, IDrawPrimitive_
    {
        ...
        public override void AI()
        {
            ...
            Dust.NewDustPerfect(Projectile.Center + Projectile.rotation.ToRotationVector2() * 3f, DustType<GlowFastDecelerate>(), Vector2.Zero, Scale: 0.35f, newColor: new Color(255, 255, 150));

            Projectile.velocity = (Projectile.velocity * 20f + (Main.player[Projectile.owner].Center - Projectile.Center).SafeNormalize(Vector2.UnitX) * 16.5f) / 21f;
        ...
        public override void OnKill(int timeLeft)
        {
            for (int i = 0; i < 15; i++)
            {
                Dust.NewDustPerfect(Projectile.Center + Main.rand.NextVector2Circular(5f, 5f), DustType<GlowFastDecelerate>(), Main.rand.NextVector2Circular(3f, 3f), 0, new Color(255, 191, 73), 0.3f);
            }
        }
