...
namespace BombusApisBee.Content.Projectiles
{
    ...
    public class HoneyBeeProj : BeeProjectile
    {
        ...
        public override string Texture => "BombusApisBee/Content/Forest/Items/HoneyBeeItem/HoneyBeeItem";
        ...
        public override void AI()
        {
            ...
            if (Defense)
            {
                ...
                if (blockDelay <= 0)
                {
                    ...
                    {
                        BeeUtils.CircleDust(Projectile.Center, 25, DustType<HoneyMetaballDustTransparent>(), 3f, 0, null, 4f);

                        for (int i = 0; i < 5; i++)
                        {
                            Dust.NewDustPerfect(projectileTarget.Center, DustType<GlowFastDecelerate>(), projectileTarget.velocity.RotatedByRandom(0.5f) * Main.rand.NextFloat(), 0, new Color(255, 200, 20), 0.4f);
                            Dust.NewDustPerfect(projectileTarget.Center, DustType<HoneyMetaballDustTransparent>(), Main.rand.NextVector2Circular(2f, 2f), 0, default, 2f).noGravity = true;
                        }

                        for (int i = 0; i < 20; i++)
                        {
                            Vector2 pos = Vector2.Lerp(Projectile.Center, projectileTarget.Center, i * 0.05f);
                            Dust.NewDustPerfect(pos, DustType<HoneyMetaballDustTransparent>(), Vector2.Zero, 0, default, 2f).noGravity = true;
                        }
                    ...
                    else
                    {
                        ...
                        if (target != null)
                        {
                            ...
                            for (int i = 0; i < 5; i++)
                            {
                                Dust.NewDustPerfect(target.Center, DustType<GlowFastDecelerate>(), target.velocity.RotatedByRandom(0.5f) * Main.rand.NextFloat(), 0, new Color(255, 200, 20), 0.4f);
                                Dust.NewDustPerfect(target.Center, DustType<HoneyMetaballDustTransparent>(), Main.rand.NextVector2Circular(2f, 2f), 0, default, 2f).noGravity = true;
                            }
            ...
            else if (Offense)
            {
                ...
                if (slamTimer > 0)
                {
                    ...
                    rotIncrease = MathHelper.Lerp(0f, MathHelper.TwoPi, EaseFunction.EaseCubicOut.Ease(1f - slamTimer / 60f)) + (oldDir == -1 ? MathHelper.Pi : 0f);
            ...
            else if (Gathering)
            {
                ...
                if (honeyTimer > 100)
                {
                    ...
                    Projectile.NewProjectile(Projectile.GetSource_FromThis(), BottleVector,
                        Projectile.DirectionTo(Owner.Center) * 5f, ProjectileType<BeeResourceIncreaseProjectile>(), 0, 0f, Owner.whoAmI, 0, Main.rand.Next(20, 25));
        ...
        private void FiringBehavior()
        {
            ...
            if (AttackDelay > 40)
            {
                if (AttackDelay >= 60)
                {
                    ...
                    Projectile.NewProjectile(Projectile.GetSource_FromAI(), pos, pos.DirectionTo(attackTarget.Center).RotatedByRandom(1f) * 5f, ProjectileType<HoneyHomingMetaballs>(), Projectile.damage / 2, Projectile.knockBack * 0.5f, Owner.whoAmI);
                    ...
                    for (float k = 0; k < 6.28f; k += 0.1f)
                    {
                        ...
                        Dust.NewDustPerfect(pos + pos.DirectionTo(attackTarget.Center) * 5f, DustType<HoneyMetaballDust>(), new Vector2(x, y).RotatedBy(Projectile.rotation + MathHelper.PiOver2) * 0.04f, 0, default, 1f).noGravity = true;
                    }
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            if (Offense && slamming)
            {
                ...
                for (int i = 0; i < 15; i++)
                {
                    ...
                    Dust.NewDustPerfect(pos, DustType<GlowFastDecelerate>(), pos.DirectionTo(Projectile.Center).RotatedByRandom(0.45f) * Main.rand.NextFloat(1f, 6f), 0, new Color(250, 200, 0), 0.65f);
                }
                ...
                for (float k = 0; k < 6.28f; k += 0.1f)
                {
                    ...
                    Dust.NewDustPerfect(target.Center + target.Center.DirectionTo(Projectile.Center) * (target.width * 0.5f), DustType<GlowFastDecelerate>(), new Vector2(x, y).RotatedBy(Projectile.rotation + MathHelper.PiOver2) * 0.05f, 0, new Color(255, 150, 0), 0.65f);
                }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            if (shieldFade > 0)
            {
                Effect effect = Filters.Scene["HoneyShieldShader"].GetShader().Shader;
                ...
                Texture2D noise = Request<Texture2D>("BombusApisBee/ExtraTextures/SwirlyNoiseLooping").Value;
