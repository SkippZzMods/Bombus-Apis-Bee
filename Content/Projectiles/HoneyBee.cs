...
namespace BombusApisBee.Content.Projectiles
{
    public class HoneyBee : BaseBeeProjectile
    {
        ...
        public override void OnKill(int timeLeft)
        {
            for (int i = 0; i < 5; i++)
            {
                Dust.NewDustPerfect(Projectile.Center, DustType<HoneyDustSolid>(), Main.rand.NextVector2Circular(1.5f, 1.5f), Main.rand.Next(100), default, 1.35f).noGravity = true;

                Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(), Main.rand.NextVector2Circular(2f, 2f), 0, Main.rand.NextBool() ? new Color(180, 90, 0) : new Color(251, 172, 17), 0.6f);
            }
        }

        public override bool SafePreDraw(ref Color lightColor)
        {
            Texture2D tex = Request<Texture2D>(Texture + (Giant ? "_Giant" : "")).Value;
            Texture2D glowTex = Request<Texture2D>("BombusApisBee/ExtraTextures/BeeGlowHM").Value;
            if (Giant)
                glowTex = Request<Texture2D>("BombusApisBee/ExtraTextures/BeeGlowGiant").Value;
