...
{
    public class HoneyEnergyBee : CommonBeeProjectile
    {
        ...
        public override void OnKill(int timeLeft)
        {
            for (int i = 0; i < 10; i++)
            {
                Dust.NewDustPerfect(Projectile.Center + Main.rand.NextVector2Circular(5f, 5f), DustType<GlowFastDecelerate>(), Main.rand.NextVector2Circular(1.5f, 1.5f), 0, new Color(255, 255, 204), 0.4f);

                Dust.NewDustPerfect(Projectile.Center + Main.rand.NextVector2Circular(5f, 5f), DustType<GlowFastDecelerate>(), Main.rand.NextVector2Circular(2.5f, 2.5f), 0, new Color(253, 232, 0), 0.3f);
            }
        }
        ...
        public override bool SafePreDraw(ref Color lightColor)
        {
            ...
            Texture2D texGiant = Request<Texture2D>(Texture + "_Giant").Value;
