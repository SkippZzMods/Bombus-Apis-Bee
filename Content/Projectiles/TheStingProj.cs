...
namespace BombusApisBee.Content.Projectiles
{
    public class TheStingProj : BeeProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Main.myPlayer == Projectile.owner && player.channel)
            {
                ...
                if (stingprojtimer < 240)
                {
                    ...
                    Projectile.velocity = (Projectile.velocity * 20f + Projectile.Center.DirectionTo(Main.MouseWorld + (Projectile.ai[1] / 16f).ToRotationVector2() * 95f).SafeNormalize(Vector2.UnitX) * 19f) / 21f;
                    Projectile.ai[1] += 1f;
                    ...
                    if (distance < 800f)
                    {
                        if (Projectile.ai[0] > 10f)
                        {
                            ...
                            shootToX *= distance * 4;
                            shootToY *= distance * 4;
                            Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center.X, Projectile.Center.Y, shootToX, shootToY, ProjectileType<StingerFriendly>(), Projectile.damage * 1 / 2, Projectile.knockBack, Main.myPlayer);
                            Projectile.ai[0] = 0f;
                        }
                    }
                }
        ...
        public override void OnKill(int timeLeft)
        {
            for (int i = 0; i < 50; i++)
            {
                int dustIndex = Dust.NewDust(new Vector2(Projectile.position.X, Projectile.position.Y), Projectile.width, Projectile.height, 31, 0f, 0f, 100, default, 1.2f);
