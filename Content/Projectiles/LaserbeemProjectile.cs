...
namespace BombusApisBee.Content.Projectiles
{
    public class LaserbeemProjectile : BeeProjectile, IDrawPrimitive_
    {
        ...
        {
            Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(), Vector2.UnitY.RotatedBy(Projectile.rotation), 0, new Color(253, 232, 0), 0.2f);
        ...
        public override void OnKill(int timeLeft)
        {
            for (int i = 0; i < 25; i++)
            {
                Dust.NewDustPerfect(Projectile.Center + Main.rand.NextVector2Circular(5f, 5f), DustType<GlowFastDecelerate>(), default, 55,
        ...
        {
            Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center, Main.rand.NextVector2Circular(6f, 6f), ProjectileType<HoneyEnergyBee>(), (int)(Projectile.damage * 0.65f), 0f, Projectile.owner);
            for (int i = 0; i < 10; i++)
            {
                Dust.NewDustPerfect(Projectile.Center + Main.rand.NextVector2Circular(5f, 5f), DustType<GlowFastDecelerate>(), default, 55,
                    Main.rand.NextBool() ? new Color(253, 232, 0) : new Color(255, 255, 204), Main.rand.NextFloat(0.4f, 0.5f));
