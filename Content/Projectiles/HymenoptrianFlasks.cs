    ...
    public class HymenoptraFlask_Honey : BeeProjectile
    {
        ...
        public override void OnKill(int timeLeft)
        {
            if (Main.myPlayer == Projectile.owner)
            {
                ...
                {
                    Projectile.NewProjectile(Projectile.GetSource_Death(), Projectile.Center, Main.rand.NextVector2Circular(5f, 5f), ProjectileType<HoneySmoke>(), Projectile.damage * 2 / 3, 0f, Projectile.owner);
                }
            ...
            for (int i = 0; i < 20; i++)
            {
                Dust.NewDustPerfect(Projectile.Center, DustType<HoneyDust>(), Main.rand.NextVector2Circular(2f, 2f), Main.rand.Next(50, 150), default, 1.5f).noGravity = true;
                ...
                Dust.NewDustPerfect(Projectile.Center, DustType<HoneyDust>(), Main.rand.NextVector2Circular(6f, 6f), Main.rand.Next(50, 150), default, 1.5f).noGravity = true;
        ...
        public override void AI()
        {
            ...
            Dust.NewDustPerfect(Projectile.Center + Main.rand.NextVector2Circular(12.5f, 12.5f), Main.rand.NextBool() ? DustID.Honey2 : DustType<HoneyDust>(), null, Main.rand.Next(100, 150), default, 1.1f).noGravity = true;
        }
    ...
    public class HymenoptraFlask_Nectar : BeeProjectile
    {
        ...
        public override void OnKill(int timeLeft)
        {
            if (Main.myPlayer == Projectile.owner)
            {
                for (int i = 0; i < 3; i++)
                {
                    Projectile.NewProjectile(Projectile.GetSource_Death(), Projectile.Center, Main.rand.NextVector2Circular(4f, 4f), ProjectileType<NectarHealingBolt>(), 0, 0f, Projectile.owner, Main.rand.Next(2, 5));
                }

                for (int i = 0; i < Main.rand.Next(2, 6); i++)
                {
                    Projectile.NewProjectile(Projectile.GetSource_Death(), Projectile.Center, Main.rand.NextVector2Circular(8f, 8f), ProjectileType<HomingNectar>(), Projectile.damage / 2, 3f, Projectile.owner, 1);
                }

                for (int i = 0; i < Main.rand.Next(8, 12); i++)
                {
                    Projectile.NewProjectile(Projectile.GetSource_Death(), Projectile.Center, Main.rand.NextVector2Circular(5f, 5f), ProjectileType<NectarSmoke>(), Projectile.damage * 2 / 3, 0f, Projectile.owner);
                }
            }

            for (int i = 0; i < 25; i++)
            {
                Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(), Main.rand.NextVector2Circular(2.5f, 2.5f), 0, new Color(206, 116, 59), 0.5f);

                Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(), Main.rand.NextVector2Circular(3f, 3f), 0, new Color(206, 116, 59, 100), 0.65f);
            }
        ...
        public override void AI()
        {
            ...
            if (Main.rand.NextBool())
                Dust.NewDustPerfect(Projectile.Center + Main.rand.NextVector2Circular(10f, 10f), DustType<GlowFastDecelerate>(), Main.rand.NextVector2Circular(1.5f, 1.5f), 0, new Color(206, 116, 59), 0.35f);
        }
    ...
    public class HymenoptraFlask_Stinger : BeeProjectile
    {
        ...
        public override void OnKill(int timeLeft)
        {
            if (Main.myPlayer == Projectile.owner)
            {
                for (int i = 0; i < Main.rand.Next(8, 12); i++)
                {
                    Projectile.NewProjectile(Projectile.GetSource_Death(), Projectile.Center, Main.rand.NextVector2Circular(5f, 5f), ProjectileType<PoisonSmoke>(), Projectile.damage * 2 / 3, 0f, Projectile.owner);
                }

                for (int i = 0; i < 6; i++)
                {
                    Vector2 velo = Vector2.UnitY.RotatedByRandom(0.7f) * -Main.rand.NextFloat(10f, 15f);
                    Projectile.NewProjectile(Projectile.GetSource_Death(), Projectile.Center, velo, ProjectileType<StingerGravity>(), Projectile.damage * 2 / 3, 3f, Projectile.owner);
                }
            }
