namespace BombusApisBee.Content.Hell.Items.SyrupSickle
{
    public class SyrupSickleProjectile : BeeProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Projectile.timeLeft > 30)
            {
                Dust.NewDustPerfect(Projectile.Center + (Projectile.rotation + MathHelper.ToRadians(30f)).ToRotationVector2() * 10f, DustType<GlowFastDecelerate>(), Vector2.Zero, Scale: 0.4f, newColor: new Color(255, 191, 93));

                Dust.NewDustPerfect(Projectile.Center + (Projectile.rotation + MathHelper.ToRadians(30f)).ToRotationVector2() * -10f, DustType<GlowFastDecelerate>(), Vector2.Zero, Scale: 0.4f, newColor: new Color(255, 191, 93));
            }
            else
            {
                Dust.NewDustPerfect(Projectile.Center + (Projectile.rotation + MathHelper.ToRadians(30f)).ToRotationVector2() * 10f, DustType<GlowFastDecelerate>(), Vector2.Zero, Scale: 0.4f, newColor: new Color(255, 191, 93) * MathHelper.Lerp(1f, 0f, 1f - Projectile.timeLeft / 30f));
