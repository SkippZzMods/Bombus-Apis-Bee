...
namespace BombusApisBee.Content.Hell.Items.HellfireBeemstick
{
    public class HellfireBeemstick : BeekeeperWeapon
    {
        ...
        public override void UseStyle(Player player, Rectangle heldItemFrame)
        {
            ...
            if (animProgress < 0.55f)
            {
                ...
                itemPosition += itemRotation.ToRotationVector2() * MathHelper.Lerp(-3f, 7f, EaseFunction.EaseCircularInOut.Ease(lerper));
            }
            else
            {
                if (animProgress < 0.75f)
                {
                    ...
                    itemPosition += itemRotation.ToRotationVector2() * MathHelper.Lerp(7f, -4f, EaseFunction.EaseQuinticInOut.Ease(lerper));
                }
                else
                {
                    float lerper = (animProgress - 0.75f) / 0.25f;
                    itemPosition += itemRotation.ToRotationVector2() * MathHelper.Lerp(-4f, 6f, EaseFunction.EaseQuinticInOut.Ease(lerper));
                }
            }
            ...
            if (Main.rand.NextBool(20))
                Dust.NewDustPerfect(dustPos, DustType<HellfireBeemstickSmokeDust>(), Vector2.UnitY * -2f, 100, default, 0.03f).noGravity = true;

            if (Main.rand.NextBool(10))
                Dust.NewDustPerfect(dustPos, DustType<GlowFastDecelerate>(), Vector2.UnitY * -2f, (int)MathHelper.Lerp(100f, 200f, animProgress), new Color(200, 75, 20), 0.5f).noGravity = true;
        ...
        public override void UseItemFrame(Player player)
        {
            ...
            if (animProgress < 0.55f)
            {
                if (animProgress < 0.1f)
                {
                    ...
                    rotation += MathHelper.Lerp(0f, -0.65f, EaseFunction.EaseCircularOut.Ease(lerper)) * player.direction;
                }
                else
                {
                    float lerper = (animProgress - 0.1f) / 0.45f;
                    rotation += MathHelper.Lerp(-0.65f, 0, EaseFunction.EaseCircularInOut.Ease(lerper)) * player.direction;
                }
            }
            else
            {
                ...
                {
                    float lerper = (animProgress - 0.55f) / 0.2f;
                    rotation += MathHelper.Lerp(0f, 0.25f, EaseFunction.EaseCircularInOut.Ease(lerper)) * player.direction;
                    if (animProgress >= 0.6f && !spawnedGore)
                    {
                        ...
                        for (int i = 0; i < 10; i++)
                        {
                            Dust.NewDustPerfect(pos, DustType<GlowFastDecelerate>(), (-(rotation + 1.5707964f * player.gravDir).ToRotationVector2() * 3f + Vector2.UnitY * -2f).RotatedByRandom(0.5f) * Main.rand.NextFloat(0.35f, 0.65f), 0, new Color(200, 75, 20), 0.45f);
                ...
                {
                    float lerper = (animProgress - 0.75f) / 0.25f;
                    rotation += MathHelper.Lerp(0.25f, 0.05f, EaseFunction.EaseCircularInOut.Ease(lerper)) * player.direction;
                }
            }
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            {
                Projectile.NewProjectileDirect(source, position, velocity * 2.5f, ProjectileType<HellfireBeemstickSlug>(), damage * 2, knockback * 3.5f, player.whoAmI);

                cooldown = 5;
            }
            else
            {
                Projectile.NewProjectileDirect(source, position, velocity, ProjectileType<HellfireBeeBuckshot>(), damage, knockback * 3.5f, player.whoAmI);

                for (int i = 0; i < Main.rand.Next(2, 4); i++)
                {
                    Projectile.NewProjectileDirect(source, position, velocity.RotatedByRandom(12f.AsRadians()), ProjectileType<HellfireBee>(), player.beeDamage((int)(damage * 0.65f)),
                        player.beeKB(1f), player.whoAmI).DamageType = BeeUtils.BeeDamageClass();
                }

                for (int i = 0; i < Main.rand.Next(3, 6); i++)
                {
                    Projectile.NewProjectileDirect(source, position, velocity.RotatedByRandom(8f.AsRadians()), ProjectileType<HellfireBeeBuckshot>(), damage, knockback * 3.5f, player.whoAmI);
                }
            }
            ...
            for (int i = 0; i < 15; i++)
            {
                Dust.NewDustPerfect(position, DustType<GlowFastDecelerate>(), velocity.RotatedByRandom(0.3f) * Main.rand.NextFloat(2f), 0, new Color(200, 75, 20), 0.65f);
            ...
            Dust dust = Dust.NewDustPerfect(position, DustType<HellfireBeemstickSmokeDust>(), Main.rand.NextVector2Circular(1f, 1f) + -velocity * 0.35f + Vector2.UnitY * -1.5f, 50, default, 0.1f);
            dust.rotation = Main.rand.NextFloat(6.28f);

            dust = Dust.NewDustPerfect(position, DustType<HellfireBeemstickSmokeDust>(), Vector2.UnitY * -1.5f, 50, default, 0.1f);
            dust.rotation = Main.rand.NextFloat(6.28f);

            dust = Dust.NewDustPerfect(position, DustType<HellfireBeemstickMuzzleFlashDust>(), Vector2.Zero, 0, default, 1f);
    ...
    public class HellfireBeemstickSmokeDust : ModDust
    {
        ...
        public override bool PreDraw(Dust dust)
        {
            ...
            Texture2D tex = Request<Texture2D>("BombusApisBee/ExtraTextures/SmokeTransparent_" + dust.customData).Value;
    ...
    class HellfireBeemstickMuzzleFlashDust : ModDust
    {
        ...
        public override bool PreDraw(Dust dust)
        {
            ...
            Texture2D tex = Request<Texture2D>("BombusApisBee/ExtraTextures/HellShotgunMuzzleFlash").Value;
            Texture2D texBlur = Request<Texture2D>("BombusApisBee/ExtraTextures/HellShotgunMuzzleFlash_Blur").Value;
            Texture2D texGlow = Request<Texture2D>("BombusApisBee/ExtraTextures/HellShotgunMuzzleFlash_Glow").Value;
