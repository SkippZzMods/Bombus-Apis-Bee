namespace BombusApisBee.Content.Hell.Items.Honeyshot
{
    ...
    {
        public override string Texture => "BombusApisBee/Content/Hell/Items/Honeyshot/HoneyShot";
        ...
        public override void AI()
        {
            ...
            if (updateTime && CurrentCharge >= ChargeTime)
            {
                if (Main.myPlayer == Projectile.owner)
                {
                    for (int i = -1; i < 2; i++)
                    {
                        Projectile.NewProjectile(Projectile.GetSource_FromAI(), owner.Center + offset * 10f * Projectile.direction, owner.DirectionTo(Main.MouseWorld).RotatedBy(0.25f * i) * 20f,
                            ProjectileType<HoneyArrow>(), Projectile.damage, Projectile.knockBack, Projectile.owner);
                    }
                }
                ...
                for (int i = 0; i < 20; i++)
                {
                    ...
                    Dust.NewDustPerfect(pos, DustType<Dusts.HoneyDustSolid>(), owner.DirectionTo(Main.MouseWorld).RotatedBy(0.65f).RotatedByRandom(0.3f) * Main.rand.NextFloat(5f), Main.rand.Next(100), default, 1.45f).noGravity = true;

                    Dust.NewDustPerfect(pos, DustType<Dusts.HoneyDustSolid>(), owner.DirectionTo(Main.MouseWorld).RotatedByRandom(0.3f) * Main.rand.NextFloat(5f), Main.rand.Next(100), default, 1.45f).noGravity = true;
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Texture2D texGlow = Request<Texture2D>("BombusApisBee/Content/Hell/Items/Honeyshot/HoneyShot_Glow").Value;
            Texture2D arrowTex = Request<Texture2D>("BombusApisBee/Content/Hell/Items/Honeyshot/HoneyArrow").Value;
            Texture2D arrowTexGlow = Request<Texture2D>("BombusApisBee/Content/Hell/Items/Honeyshot/HoneyArrow_Glow").Value;
        ...
        private void UpdateProjectileHeldVariables(Vector2 armPosition)
        {
            ...
            Projectile.position = armPosition - Projectile.Size * 0.5f + Projectile.velocity.SafeNormalize(owner.direction * Vector2.UnitX) * 15f;
    ...
    class HoneyArrow : BeeProjectile
    {
        ...
        public override void AI()
        {
            ...
            Dust.NewDustPerfect(Projectile.Center, DustType<Dusts.GlowFastDecelerate>(), Main.rand.NextVector2Circular(1.5f, 1.5f), 0, Main.rand.NextBool() ? new Color(180, 90, 0) : new Color(251, 172, 17), 0.45f);
        }
        ...
        public override void OnKill(int timeLeft)
        {
            for (int i = 0; i < 15; i++)
            {
                Dust.NewDustPerfect(Projectile.Center, DustType<Dusts.HoneyDustSolid>(), Main.rand.NextVector2Circular(2.5f, 2.5f), Main.rand.Next(100), default, 1.35f).noGravity = true;

                Dust.NewDustPerfect(Projectile.Center, DustType<Dusts.GlowFastDecelerate>(), Main.rand.NextVector2Circular(3f, 3f), 0, Main.rand.NextBool() ? new Color(180, 90, 0) : new Color(251, 172, 17), 0.6f);
            }

            for (int i = 0; i < Main.rand.Next(1, 3); i++)
            {
                Projectile.NewProjectile(Projectile.GetSource_Death(), Projectile.Center, Main.rand.NextVector2CircularEdge(6f, 6f), ProjectileType<HoneyBee>(), (int)(Projectile.damage * 0.65f), 0f, Projectile.owner);
            }
