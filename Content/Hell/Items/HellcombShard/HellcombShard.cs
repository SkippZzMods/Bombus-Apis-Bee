...
namespace BombusApisBee.Content.Hell.Items.HellcombShard
{
    public class HellcombDust : ModDust
    {
        ...
        public override bool PreDraw(Dust dust)
        {
            ...
            effect.Parameters["noiseImage1"].SetValue(Request<Texture2D>("BombusApisBee/ExtraTextures/PerlinNoise").Value);
    ...
    public class HellcombShard : BeeKeeperItem
    {
        ...
        private void SpawnExplosion(Projectile proj, int timeLeft)
        {
            if (Main.player[proj.owner].Bombus().HasHellcombShard && Main.rand.NextBool(15))
            {
                ...
                for (int i = 0; i < 20; i++)
                {
                    Dust.NewDustPerfect(proj.Center, DustType<ExplosionDust>(), Main.rand.NextVector2Circular(20f, 20f), 150 + Main.rand.Next(60), default, Main.rand.NextFloat(0.9f, 1.5f)).rotation = Main.rand.NextFloat(6.28f);

                    Dust.NewDustPerfect(proj.Center, DustType<ExplosionDustTwo>(), Main.rand.NextVector2Circular(20f, 20f), 50 + Main.rand.Next(80), default, Main.rand.NextFloat(0.9f, 1.5f)).rotation = Main.rand.NextFloat(6.28f);

                    Dust.NewDustPerfect(proj.Center, DustType<ExplosionDust>(), Main.rand.NextVector2Circular(20f, 20f), 150 + Main.rand.Next(50), default, Main.rand.NextFloat(0.9f, 1.5f)).rotation = Main.rand.NextFloat(6.28f);
                }

                for (int i = 0; i < 25; i++)
                {
                    Dust.NewDustPerfect(proj.Center, DustType<GlowFastDecelerate>(), Main.rand.NextVector2Circular(15f, 15f), 0, new Color(255, 200, 0, 0), 0.6f);
                }
            }
        }
    ...
    class HellcombShardExplosion : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            for (int k = 0; k < 6; k++)
            {
                ...
                {
                    Dust.NewDustPerfect(Projectile.Center + Vector2.One.RotatedBy(rot) * Radius, DustType<HellcombDust>(),
