        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            for (int i = 0; i < 2; i++)
            {
                if (Main.myPlayer == Projectile.owner)
                {
                    ...
                    Projectile.NewProjectile(Projectile.GetSource_OnHit(target), target.Center.X, target.Center.Y, value15.X, value15.Y, ProjectileType<GalacticSeeker>(), hit.Damage * 1 / 4, hit.Knockback, Projectile.owner, 0f, 0f);
                }
            }
        }
    }
