...
namespace BombusApisBee.Content.Pillars.Items.GalaxyHoneycomb
{
    ...
    {
        public override string Texture => "BombusApisBee/Content/Pillars/Items/GalaxyHoneycomb/HoneycombOfTheGalaxies";
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            for (int k = 0; k < Projectile.oldPos.Length; k++)
            {
                ...
                float completionRatio = k / (float)Projectile.oldPos.Length;
                Color drawColor = new Color(Main.DiscoR, Main.DiscoG, Main.DiscoB, Projectile.alpha) * ((Projectile.oldPos.Length - k) / (float)Projectile.oldPos.Length);
            ...
            if (StarTimer >= 10f)
            {
                if (Main.myPlayer == Projectile.owner)
                {
                    ...
                    for (int i = 0; i < 2; i++)
                    {
                        ...
                        {
                            vel = new Vector2(Main.rand.Next(-100, 101), Main.rand.Next(-100, 101));
                        }
                        vel.Normalize();
                        vel *= Main.rand.Next(70, 101) * 0.1f;
                        Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center, vel, ProjectileType<GalacticStar>(), Projectile.damage * 1 / 2, Projectile.knockBack, Projectile.owner);
                    }
                    if (HasScaled)
                    {
                        Vector2 vel = new Vector2(Main.rand.Next(-100, 101), Main.rand.Next(-100, 101));
                        ...
                        vel.Normalize();
                        vel *= Main.rand.Next(70, 101) * 0.1f;
                        Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center, vel, ProjectileType<GalacticBee>(), Projectile.damage * 1 / 2, Projectile.knockBack, Projectile.owner);
                    }
                }
            ...
            if (Time > 180f)
            {
                if (!HasScaled)
                {
                    ...
                    for (int i = 0; i < Repeats; ++i)
                    {
                        ...
                        Dust dust3 = Dust.NewDustPerfect(Projectile.Center, 27, null, 50, default, 2.1f);
                        dust3.velocity = angle2.ToRotationVector2() * 5f;
                        dust3.noGravity = true;
                    }
                    const int Repeats2 = 65;
                    for (int i = 0; i < Repeats2; ++i)
                    {
                        float angle2 = 6.2831855f * i / Repeats2;
                        Dust dust3 = Dust.NewDustPerfect(Projectile.Center, 58, null, 50, default, 2.3f);
                        dust3.velocity = angle2.ToRotationVector2() * 6f;
                        dust3.noGravity = true;
                    }
                    const int Repeats3 = 65;
                    for (int i = 0; i < Repeats3; ++i)
                    {
                        float angle2 = 6.2831855f * i / Repeats3;
                        Dust dust3 = Dust.NewDustPerfect(Projectile.Center, 59, null, 50, default, 2.5f);
                        dust3.velocity = angle2.ToRotationVector2() * 7f;
                        dust3.noGravity = true;
                    }
                    for (int i = 0; i < 12; i++)
                    {
                    ...
                    for (int i = 0; i < 6; i++)
                    {
                        if (Main.myPlayer == Projectile.owner)
                        {
                            ...
                            value15.Normalize();
                            value15 *= Main.rand.Next(70, 101) * 0.1f;
                            Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center.X, Projectile.Center.Y, value15.X, value15.Y, ProjectileType<GalacticSeeker>(), Projectile.damage, Projectile.knockBack, Projectile.owner, 0f, 0f);
                        }
                    }
        ...
        internal void SpawnDeathProjectiles()
        {
            for (int i = 0; i < (Time > 240f ? 8 : 4); i++)
            {
                if (Main.myPlayer == Projectile.owner)
                {
                    ...
                    Projectile.NewProjectile(Projectile.GetSource_Death(), Projectile.position, velocity.RotatedByRandom(35), ProjectileType<HealingProjectile>(), 0, 0f, Projectile.owner, 0, 6);
                }
            }
            for (int i = 0; i < 5; i++)
            {
                ...
                {
                    Vector2 velocity = Projectile.SafeDirectionTo(Owner.Center, null) * 22f;
                    Projectile.NewProjectile(Projectile.GetSource_Death(), Projectile.position, velocity.RotatedByRandom(8), ProjectileType<GalacticBee>(), Projectile.damage * 1 / 2, 0f, Projectile.owner);
                }
            }
            for (int i = 0; i < 55; i++)
            {
        ...
        internal void ReturnToOwner()
        {
            ...
            Projectile.velocity = Projectile.SafeDirectionTo(Owner.Center, null) * 22f;
            if (Projectile.Hitbox.Intersects(Owner.Hitbox))
            {
                ...
                {
                    float angle2 = 6.2831855f * i / Repeats;
                    Dust dust3 = Dust.NewDustPerfect(Owner.Center, DustID.Shadowflame, null, 50, default, 1.1f);
                    dust3.velocity = angle2.ToRotationVector2() * 6f;
                    dust3.noGravity = true;
                }
                Projectile.Kill();
