...
{
    public class GalacticBee : CommonBeeProjectile
    {
        ...
        public override void OnKill(int timeLeft)
        {
            Player player = Main.player[Projectile.owner];
            {
                if (Main.myPlayer == Projectile.owner)
                {
                    ...
                    float num427 = Projectile.position.X + Projectile.width / 2 - vector7.X;
                    float num428 = Projectile.position.Y + Projectile.height / 2 - vector7.Y;
                    int num429 = 31;
                    float num430 = (float)Math.Sqrt(num427 * num427 + num428 * num428);
                    num430 = num429 / num430;
                    num427 *= num430;
                    num428 *= num430;
                    int num431 = Projectile.damage;
                    int num432 = Projectile.NewProjectile(Projectile.GetSource_Death(), x, y, num427, num428, ProjectileType<LargeGalaticStar>(), num431, Projectile.knockBack, Projectile.owner);
                }
            }
        }
        public override void SafeAI()
        {
            ...
            Dust.NewDust(Projectile.position, 14, 14, dustType, Projectile.velocity.X * 0.1f, Projectile.velocity.Y * 0.1f, 150, default, 0.5f);
        }
    }
}
