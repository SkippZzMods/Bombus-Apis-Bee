namespace BombusApisBee.Content.Forest.Items.Honeycomb
{
    public class HoneycombHoldout : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (shots < 3)
            {
                ...
                if (++shootDelay >= MaxDelay)
                {
                    ...
                    for (int i = 0; i < 15; i++)
                    {
                        Dust.NewDustPerfect(barrelPos, DustType<Dusts.HoneyDust>(), Projectile.velocity.RotatedByRandom(0.75f) * Main.rand.NextFloat(0.5f, 6f), Main.rand.Next(85), Scale: Main.rand.NextFloat(0.8f, 1.2f)).noGravity = true;
                    }
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            for (int i = 0; i < 15; i++)
            {
                Dust.NewDustDirect(Projectile.position, Projectile.width, Projectile.height, DustType<Dusts.HoneyDust>(), Alpha: Main.rand.Next(100));
            }
