    ...
    public class BeezookaRocket : BeeProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (exploding)
            {
                ...
                Dust.NewDustPerfect(pos, DustType<GlowFastDecelerate>(), pos.DirectionTo(Projectile.Center) * 2.5f, 0, new Color(214, 158, 79), 0.35f);
                pos = Projectile.Center + Main.rand.NextVector2CircularEdge(lerper, lerper);
                Dust.NewDustPerfect(pos, DustType<GlowFastDecelerate>(), pos.DirectionTo(Projectile.Center) * 2.5f, 0, new Color(255, 218, 110), 0.35f);
            }
            else
            {
                ...
                if (foundTarget)
                    Projectile.velocity = (Projectile.velocity * 30f + (targetCenter - Projectile.Center).SafeNormalize(Vector2.UnitX) * 25f) / 31f;
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            if (!exploding)
            {
                ...
                for (int i = 0; i < 15; i++)
                {
                    Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(), Projectile.velocity.RotatedByRandom(0.45f) * Main.rand.NextFloat(), 0, new Color(214, 158, 79), 0.45f);

                    Dust.NewDustPerfect(Projectile.Center, DustType<HoneyDustSolid>(), Projectile.velocity.RotatedByRandom(0.65f) * Main.rand.NextFloat(0.25f), 50 + Main.rand.Next(100), default, 1.45f).noGravity = true;
                }
        ...
        public override void OnKill(int timeLeft)
        {
            if (exploding)
            {
                for (int i = 0; i < 10; i++)
                {
                    Dust.NewDustPerfect(Projectile.Center, DustType<ExplosionDust>(), Main.rand.NextVector2Circular(5f, 5f), 50 + Main.rand.Next(100), default, 0.9f).rotation = Main.rand.NextFloat(6.28f);

                    Dust.NewDustPerfect(Projectile.Center, DustType<ExplosionDustTwo>(), Main.rand.NextVector2Circular(6f, 6f), 80 + Main.rand.Next(100), default, 0.6f).rotation = Main.rand.NextFloat(6.28f);

                    Dust.NewDustPerfect(Projectile.Center, DustType<ExplosionDustThree>(), Main.rand.NextVector2Circular(6f, 6f), 80 + Main.rand.Next(100), default, 0.9f).rotation = Main.rand.NextFloat(6.28f);

                    Dust.NewDustPerfect(Projectile.Center, DustType<ExplosionDustFour>(), Main.rand.NextVector2Circular(5f, 5f), 50 + Main.rand.Next(100), default, 0.6f).rotation = Main.rand.NextFloat(6.28f);

                    Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(), Main.rand.NextVector2Circular(8f, 8f), 0, new Color(214, 158, 79), 0.55f);

                    Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(), Main.rand.NextVector2Circular(8f, 8f), 0, new Color(255, 218, 110), 0.45f);

                    Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(), (Projectile.rotation - MathHelper.PiOver2).ToRotationVector2().RotatedByRandom(0.45f) * Main.rand.NextFloat(10f, 20f), 0, new Color(214, 158, 79), 0.45f);
                }
                ...
                Projectile.NewProjectile(Projectile.GetSource_Death(), Projectile.Center, Vector2.Zero, ProjectileType<BasicExplosion>(), Projectile.damage, 2f, Projectile.owner);

                for (int i = 0; i < Main.rand.Next(1, 3); i++)
                {
                    Projectile.NewProjectile(Projectile.GetSource_Death(), Projectile.Center, (Projectile.rotation - MathHelper.PiOver2).ToRotationVector2().RotatedByRandom(0.45f) * Main.rand.NextFloat(10f, 20f), ProjectileType<BeezookaRocketSmall>(), (int)(Projectile.damage * 0.65f), 3f, Projectile.owner);
                }
            }
        }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Texture2D tex = Request<Texture2D>(Texture + alt).Value;
            Texture2D texGlow = Request<Texture2D>(Texture + alt + "_Glow").Value;
    ...
    public class BeezookaRocketSmall : BeeProjectile
    {
        ...
        public override void AI()
        {
            if (Main.myPlayer == Projectile.owner)
            {
                ...
                if (target != default)
                    Projectile.velocity = (Projectile.velocity * 22f + (target.Center - Projectile.Center).SafeNormalize(Vector2.UnitX) * 30f) / 23f;
            }

            Projectile.rotation = Projectile.velocity.ToRotation() + MathHelper.PiOver2;

            Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(), Vector2.Zero, 0, new Color(214, 158, 79), 0.3f);
        }
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            for (int i = 0; i < Main.rand.Next(1, 3); i++)
            {
                Projectile.NewProjectile(Projectile.GetSource_Death(), Projectile.Center, Projectile.velocity.RotatedByRandom(0.4f) * Main.rand.NextFloat(0.75f, 2f), ProjectileType<BeezookaShrapnel>(), (int)(Projectile.damage * 0.25f), 3.5f, Projectile.owner);
            }

            for (int i = 0; i < 10; i++)
            {
                Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(), Projectile.velocity.RotatedByRandom(0.45f) * Main.rand.NextFloat(), 0, new Color(214, 158, 79), 0.35f);

                Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(), Main.rand.NextVector2Circular(2.5f, 2.5f), 0, new Color(214, 158, 79), 0.35f);
            }
        }
