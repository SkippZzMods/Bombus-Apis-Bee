...
namespace BombusApisBee.Content.Forest.Items.Beezooka
{
    public class BeeZooka : BeeDamageItem
    {
        ...
        public override void SafeSetDefaults()
        {
            ...
            Item.autoReuse = true;
            Item.shoot = ProjectileType<BeezookaRocket>();
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            for (int i = 0; i < 20; i++)
            {
                Dust.NewDustPerfect(position, DustType<Dusts.HoneyDustSolid>(), velocity.RotatedByRandom(0.45f) * Main.rand.NextFloat(0.5f), 0, default, 1.25f).noGravity = true;

                Dust.NewDustPerfect(position, DustType<Dusts.GlowFastDecelerate>(), velocity.RotatedByRandom(0.35f) * Main.rand.NextFloat(0.45f), 0, new Color(163, 97, 66), 0.55f);
            ...
            for (int i = 0; i < 5; i++)
            {
                Dust.NewDustPerfect(position, DustType<Dusts.ExplosionDust>(), velocity.RotatedByRandom(0.35f) * Main.rand.NextFloat(0.1f, 1f), 50 + Main.rand.Next(100), default, 1f).rotation = Main.rand.NextFloat(6.28f);

                Dust.NewDustPerfect(position, DustType<Dusts.ExplosionDustTwo>(), velocity.RotatedByRandom(0.35f) * Main.rand.NextFloat(0.1f, 1f), 80 + Main.rand.Next(100), default, 1f).rotation = Main.rand.NextFloat(6.28f);

                Dust.NewDustPerfect(position, DustType<Dusts.ExplosionDustThree>(), velocity.RotatedByRandom(0.35f) * Main.rand.NextFloat(0.1f, 1f), 80 + Main.rand.Next(100), default, 1f).rotation = Main.rand.NextFloat(6.28f);

                Dust.NewDustPerfect(position, DustType<Dusts.ExplosionDustFour>(), velocity.RotatedByRandom(0.35f) * Main.rand.NextFloat(0.1f, 1f), 50 + Main.rand.Next(100), default, 1f).rotation = Main.rand.NextFloat(6.28f);
            }
        ...
        {
            CreateRecipe(1).
                AddIngredient(ItemType<BeenadeLauncher.BeenadeLauncher>()).
