...
namespace BombusApisBee.Content.Forest.Items.GelatinousHoneycomb
{
    public class GelBee : BaseBeeProjectile
    {
        ...
        public override void OnKill(int timeLeft)
        {
            if (Main.myPlayer == Projectile.owner && Main.rand.NextBool(3))
            {
                ...
                Projectile.NewProjectile(Projectile.GetSource_Death(), pos, pos.DirectionTo(Projectile.Center) * 7f, ProjectileType<VolatileGel>(), Projectile.damage, 0f, Projectile.owner);
            }
        }
    ...
    class VolatileGel : BeeProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Main.rand.NextBool(3))
                Dust.NewDustPerfect(Projectile.Center + Main.rand.NextVector2Circular(10f, 10f), DustType<Dusts.SlimeDust>(), Main.rand.NextVector2Circular(1f, 1f), Main.rand.Next(50, 100), default, 0.9f).noGravity = true;
        }
        ...
        public override bool OnTileCollide(Vector2 oldVelocity)
        {
            ...
            for (float k = 0; k < 6.28f; k += 0.1f)
            {
                ...
                Dust.NewDustPerfect(Projectile.Bottom, DustType<Dusts.SlimeDust>(), new Vector2(x, y).RotatedBy(oldVelocity.ToRotation() + MathHelper.PiOver2) * 0.1f, Main.rand.Next(50, 100), default, 1.25f).noGravity = true;
            }

            for (int i = 0; i < 10; i++)
            {
                Dust.NewDustPerfect(Projectile.Center, DustType<Dusts.SlimeDust>(), Main.rand.NextVector2Circular(3f, 3f), Main.rand.Next(50, 100), default, 1.35f).noGravity = true;
            }
            return false;
        }
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            for (int i = 0; i < 20; i++)
            {
                Dust.NewDustPerfect(Projectile.Center, DustType<Dusts.SlimeDust>(), Main.rand.NextVector2Circular(2f, 2f), Main.rand.Next(50, 100), default, 1.35f).noGravity = true;
            }
        }
    ...
    class RubyProjectile : BeeProjectile
    {
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            for (int i = 0; i < 25; i++)
            {
                Dust.NewDustPerfect(Projectile.Center + Main.rand.NextVector2Circular(5.5f, 5.5f), DustType<Dusts.GlowFastDecelerate>(), Main.rand.NextVector2Circular(3f, 3f), 0,
                    Main.rand.NextBool() ? new Color(243, 172, 140) : new Color(212, 37, 24), Main.rand.NextFloat(0.6f, 0.75f));
            }

            if (Main.myPlayer == Projectile.owner)
                Projectile.NewProjectile(Projectile.GetSource_Death(), Projectile.Center, Vector2.Zero, ProjectileType<RubyExplosion>(), Projectile.damage, 0f, Projectile.owner, 35);
        }
