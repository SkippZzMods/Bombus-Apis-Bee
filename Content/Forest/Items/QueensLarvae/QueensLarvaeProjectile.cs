...
namespace BombusApisBee.Content.Forest.Items.QueensLarvae
{
    ...
    public class QueensLarvaeProjectile : BeeProjectile
    {
        ...
        private void StingerRapidAI(NPC target)
        {
            ...
            Projectile.velocity = (Projectile.velocity * 20f + Projectile.Center.DirectionTo(target.Center + (++switchTimer / 16f).ToRotationVector2() * 200f).SafeNormalize(Vector2.UnitX) * 19f) / 21f;
            ...
            if (++attackTimer % 11 == 0)
                if (Collision.CanHit(Projectile.Center, 1, 1, target.Center, 1, 1))
                {
                    ...
                    if (Main.myPlayer == Projectile.owner)
                        Projectile.NewProjectile(Projectile.GetSource_FromAI(), pos, pos.DirectionTo(target.Center) * 20f, ProjectileType<StingerFriendly>(), Projectile.damage, 4.5f, Projectile.owner);
                    for (int i = 0; i < 45; ++i)
                    {
                        float angle2 = 6.2831855f * i / 45;
                        Dust.NewDustPerfect(pos, DustType<StingerDust>(), angle2.ToRotationVector2() * 3.5f, Main.rand.Next(50, 150), default, 1.15f).noGravity = true;
                    }

                    for (int i = 0; i < 30; i++)
                    {
                        Dust.NewDustPerfect(pos, DustType<StingerDust>(), pos.DirectionTo(target.Center).RotatedByRandom(0.35f) * Main.rand.NextFloat(6.5f), Main.rand.Next(50, 150), default, 1.25f).noGravity = true;

                        Dust.NewDustPerfect(pos, DustType<StingerDust>(), Main.rand.NextVector2Circular(8.5f, 8.5f), Main.rand.Next(50, 150), default, 1.25f).noGravity = true;
                    }
                }
        ...
        private void BeesAI(NPC target)
        {
            ...
            if (++attackTimer % 8 == 0)
            {
                ...
                for (int i = 0; i < 20; i++)
                {
                    Dust.NewDustPerfect(pos, DustType<HoneyDust>(), Main.rand.NextVector2Circular(1f, 1f), Main.rand.Next(150, 200), default, 1.25f).noGravity = true;
                    ...
                    Dust.NewDustPerfect(pos, DustType<HoneyDust>(), pos.DirectionTo(target.Center).RotatedByRandom(0.45f) * Main.rand.NextFloat(3f), Main.rand.Next(150, 200), default, 1.25f).noGravity = true;
        ...
        private void DashAI(NPC target)
        {
            ...
            else
            {
                ...
                if (Main.myPlayer == Projectile.owner)
                    for (int i = 0; i < 10; i++)
                    {
                        Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center, Main.rand.NextVector2Circular(15f, 15f), ProjectileType<HoneyHoming>(), Projectile.damage, 5f, Projectile.owner);
                    }

                for (int i = 0; i < 65; ++i)
                {
                    float angle2 = 6.2831855f * i / 65;
                    Dust.NewDustPerfect(Projectile.Center, DustID.Honey2, angle2.ToRotationVector2() * 9.5f, 0, default, 1.85f).noGravity = true;
                }

                for (int i = 0; i < 30; i++)
                {
                    Dust.NewDustPerfect(Projectile.Center, DustType<HoneyDust>(), Main.rand.NextVector2Circular(5f, 5f), Main.rand.Next(100, 200), default, 1.45f).noGravity = true;
        ...
        private void StingerSpreadAI(NPC target)
        {
            ...
            else
            {
                ...
                if (Main.myPlayer == Projectile.owner)
                {
                    for (int i = 0; i < 8; i++)
                    {
                        float angle2 = 6.2831855f * i / 8;
                        Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center, angle2.ToRotationVector2() * 10f, ProjectileType<HomingStinger>(), Projectile.damage * 2 / 3, 3f, owner.whoAmI);
                    }

                    for (int i = 0; i < 6; i++)
                    {
                        float radians = MathHelper.Lerp(-0.3f, 0.3f, i / (float)6);
                        Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center, Projectile.DirectionTo(target.Center).RotatedBy(radians) * 15f, ProjectileType<StingerFriendly>(), Projectile.damage * 2 / 3, 4f, owner.whoAmI);
                    }
                }

                for (int i = 0; i < 45; i++)
                {
                    Dust.NewDustPerfect(Projectile.Center, DustType<StingerDust>(), Main.rand.NextVector2Circular(6f, 6f), Main.rand.Next(100, 200), default, 1.45f).noGravity = true;
        ...
        private void HoneyBeamAI(NPC target)
        {
            ...
            if (++attackTimer < 120)
            {
                for (int i = 0; i < 3; i++)
                {
                    ...
                    Dust.NewDustPerfect(pos, DustType<Glow>(), pos.DirectionTo(Projectile.Center), 0, new Color(214, 178, 36), 0.45f);
                }
            ...
            else
            {
                ...
                Dust.NewDustPerfect(lerpPos, DustType<GlowFastDecelerate>(), Main.rand.NextVector2Circular(2f, 2f), 0, new Color(214, 178, 36) * TrailFade(), 0.45f);
                lerpPos = Vector2.Lerp(Projectile.Center + new Vector2(15 * Projectile.spriteDirection, 15), laserEnd, Main.rand.NextFloat());
                Dust.NewDustPerfect(lerpPos, DustType<Glow>(), Main.rand.NextVector2Circular(2f, 2f), 0, new Color(254, 255, 204) * TrailFade(), 0.4f);

                Dust.NewDustPerfect(Projectile.Center + new Vector2(15 * Projectile.spriteDirection, 15), DustType<GlowFastDecelerate>(), Main.rand.NextVector2Circular(5f, 5f), 0, new Color(254, 255, 204) * TrailFade(), 0.4f);

                Dust.NewDustPerfect(Projectile.Center + new Vector2(15 * Projectile.spriteDirection, 15), DustType<GlowFastDecelerate>(), Main.rand.NextVector2Circular(5f, 5f), 0, new Color(214, 178, 36) * TrailFade(), 0.4f);

                Dust.NewDustPerfect(Projectile.Center + new Vector2(15 * Projectile.spriteDirection, 15), DustType<Glow>(),
