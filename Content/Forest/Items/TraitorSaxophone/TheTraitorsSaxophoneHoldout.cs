...
namespace BombusApisBee.Content.Forest.Items.TraitorSaxophone
{
    public class TheTraitorsSaxophoneHoldout : BeeProjectile
    {
        ...
        public override string Texture => "BombusApisBee/Content/Forest/Items/TraitorSaxophone/TheTraitorsSaxophone";
        ...
        public override void AI()
        {
            ...
            if (Charge % MaxCharge == 0)
            {
                if (owner.UseBeeResource((Main.player[Projectile.owner].HeldItem.ModItem as BeekeeperWeapon).honeyCost))
                {
                    ...
                    int projType = Main.rand.Next(new int[] { ProjectileType<MiniHoneyNoteQuarter>(), ProjectileType<MiniHoneyNoteEighth>(), ProjectileType<MiniHoneyNoteEighthTied>() });
                    ...
                    for (float k = 0; k < 6.28f; k += 0.1f)
                    {
                        ...
                        Dust.NewDustPerfect(pos, DustType<HoneyDustSolid>(), new Vector2(x, y).RotatedBy(Projectile.rotation + 0.5f * Projectile.direction) * 0.045f, 75, default, 1.1f).noGravity = true;
                    }

                    for (int i = 0; i < 25; i++)
                    {
                        Vector2 pos = armPos + new Vector2(14, 10 * Projectile.direction).RotatedBy(Projectile.rotation);
                        Dust.NewDustPerfect(pos, DustType<HoneyDustSolid>(), Vector2.UnitY.RotatedBy(0.5f * Projectile.direction).RotatedByRandom(0.45f) * -Main.rand.NextFloat(5f), Main.rand.Next(50, 150), default, 1.25f).noGravity = true;
                    }
                }
                else
                    Projectile.Kill();
