namespace BombusApisBee.Content.Forest.Items.TraitorSaxophone
{
    public class MiniHoneyNoteQuarter : BeeProjectile
    {
        ...
        public override void AI()
        {
            ...
            Dust.NewDustPerfect(Projectile.Center, DustType<HoneyDustSolid>(), Vector2.Zero, 100, default, 0.8f).noGravity = true;
        }
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                Dust.NewDustPerfect(Projectile.Center, DustType<HoneyDustSolid>(), Main.rand.NextVector2Circular(3f, 3f), Main.rand.Next(50, 150), default, 1.1f).noGravity = true;
                Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(), Main.rand.NextVector2Circular(2.5f, 2.5f), 0, new Color(229, 114, 0), 0.3f).noGravity = true;
            }
        }
    ...
    public class MiniHoneyNoteEighth : BeeProjectile
    {
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            for (int i = 0; i < 3; i++)
            {
                Dust.NewDustPerfect(Projectile.Center, DustType<HoneyDustSolid>(), Main.rand.NextVector2Circular(2f, 2f), Main.rand.Next(50, 150), default, 1.1f).noGravity = true;
                Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(), Main.rand.NextVector2Circular(1.5f, 1.5f), 0, new Color(229, 114, 0), 0.3f).noGravity = true;
            }

            for (int i = 0; i < 10; i++)
            {
                Dust.NewDustPerfect(Projectile.Center, DustType<HoneyDustSolid>(), Main.rand.NextVector2CircularEdge(3f, 3f), Main.rand.Next(50, 150), default, 1.1f).noGravity = true;

                Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(), Main.rand.NextVector2CircularEdge(2.5f, 2.5f), 0, new Color(229, 114, 0), 0.3f).noGravity = true;
            }
        }
    ...
    public class MiniHoneyNoteEighthTied : BeeProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (target != default)
                Projectile.velocity = (Projectile.velocity * 25f + (target.Center - Projectile.Center).SafeNormalize(Vector2.UnitX) * 15f) / 26f;
            else
            {
