...
namespace BombusApisBee.Content.Forest.Items.TrueNectarSlasher
{
    public class TrueNectarSlasherHoldout : BeeProjectile
    {
        ...
        public override string Texture => "BombusApisBee/Content/Forest/Items/TrueNectarSlasher/TrueNectarSlasher";
        ...
        private void UpSwing()
        {
            ...
            Dust.NewDustPerfect(swordLength, DustType<GlowFastDecelerate>(), null, 0, new Color(214, 158, 79, 50), 0.35f);
        ...
        private void HeavySwing()
        {
            ...
            if (++chargeTimer < maxTimeLeft * 1.5f)
            {
                ...
                for (int i = 0; i < 3; i++)
                {
                    ...
                    Dust.NewDustPerfect(owner.Center + Projectile.rotation.ToRotationVector2() * 25f + Main.rand.NextVector2CircularEdge(lerper, lerper), DustType<Glow>(), Vector2.Zero, 0, new Color(214, 158, 79), 0.55f);
                }
            ...
            else
            {
                ...
                if (Projectile.timeLeft % Math.Floor(maxTimeLeft / 5) == 0 && Main.myPlayer == Projectile.owner)
                {
                    ...
                    Projectile.NewProjectile(Projectile.GetSource_FromAI(), owner.Center + Projectile.rotation.ToRotationVector2() * 40f, Projectile.rotation.ToRotationVector2() * 15f, ProjectileType<TrueHomingNectar>(), (int)(Projectile.damage * 0.25f), 3f, Projectile.owner);
                }

                Vector2 swordLength = Vector2.Lerp(owner.Center + Projectile.rotation.ToRotationVector2() * (20f * Projectile.scale), owner.Center + Projectile.rotation.ToRotationVector2() * (70f * Projectile.scale), Main.rand.NextFloat());
                Dust.NewDustPerfect(swordLength, DustType<GlowFastDecelerate>(), null, 0, new Color(214, 158, 79, 50), 0.45f);
        ...
        private void Thrown()
        {
            ...
            else
            {
                ...
                if (++boomerangTimer > 30f)
                {
                    ...
                    {
                        BeeUtils.DrawDustImage(owner.Center + new Vector2(3f, 0f), DustType<GlowFastDecelerate>(), 0.15f, Request<Texture2D>("BombusApisBee/ExtraTextures/TrueNectarSlasherDustImage").Value, 1f, 0, new Color(255, 255, 150), rot: 0f);

                        for (int i = 0; i < 60; ++i)
                        {
                            float angle2 = 6.28f * i / 60;
                            Dust.NewDustPerfect(owner.Center, DustType<GlowFastDecelerate>(), angle2.ToRotationVector2() * 6.75f, 0, new Color(255, 255, 150), 1.45f);
                        }
                        Projectile.Kill();
                    }

                    Dust.NewDustPerfect(Projectile.Center + (Projectile.rotation - MathHelper.PiOver4).ToRotationVector2() * (20f * Projectile.scale), DustType<GlowFastDecelerate>(), null, 0, new Color(214, 158, 79, 50), 0.55f);
                }
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            switch (Combo)
            {
                case 0: //down
                    ...
                    for (int i = 0; i < 20; i++)
                    {
                        Dust.NewDustPerfect(owner.Center + Projectile.rotation.ToRotationVector2() * (35f * Projectile.scale) + Main.rand.NextVector2Circular(5f, 5f), DustType<GlowFastDecelerate>(), Main.rand.NextVector2Circular(3.5f, 3.5f), 0, new Color(255, 191, 73), 0.3f);
                    }
                    if (!spawnedProj)
                    {
                        spawnedProj = true;
                        Projectile.NewProjectile(Projectile.GetSource_OnHit(target), pos, pos.DirectionTo(target.Center) * 25f, ProjectileType<TrueNectarslash>(), (int)(Projectile.damage * 1.05f), 2f, Projectile.owner);
                    }
                    break;
                case 1: //up
                    owner.Heal(Main.rand.Next(1, 3));
                ...
                case 2: //heavy
                    ...
                    for (int i = 0; i < 30; i++)
                    {
                        Dust.NewDustPerfect(Projectile.Center + Main.rand.NextVector2Circular(5f, 5f), DustType<GlowFastDecelerate>(), Main.rand.NextVector2Circular(4.5f, 4.5f), 0, new Color(255, 191, 73), 0.4f);
                    }
                    if (!spawnedProj)
                    {
                        spawnedProj = true;
                        for (int i = 0; i < 2; i++)
                        {
                            ...
                            Projectile.NewProjectile(Projectile.GetSource_OnHit(target), pos, pos.DirectionTo(target.Center) * 25f, ProjectileType<TrueNectarslash>(), (int)(Projectile.damage * 1.35f), 2f, Projectile.owner);
                        }
                    }
                    break;
                case 3: //throw
                    ...
                    for (int i = 0; i < 20; i++)
                    {
                        Dust.NewDustPerfect(Projectile.Center + Main.rand.NextVector2Circular(5f, 5f), DustType<GlowFastDecelerate>(), Main.rand.NextVector2Circular(3.5f, 3.5f), 0, new Color(255, 191, 73), 0.3f);
                    }
                    Projectile.NewProjectile(Projectile.GetSource_OnHit(target), pos, pos.DirectionTo(target.Center) * 25f, ProjectileType<TrueNectarslash>(), Projectile.damage, 2f, Projectile.owner);
                    break;
                case 4: //heavy
                    owner.Heal(4);
                    for (int i = 0; i < 30; i++)
        ...
        {
            DrawPrimitives();
            Texture2D sword = Request<Texture2D>(Texture).Value;
            Texture2D bladeGlow = Request<Texture2D>(Texture + "Glowy").Value;
    ...
    class TrueNectarslash : BeeProjectile
    {
        ...
        public override void PostDraw(Color lightColor)
        {
            ...
            Texture2D star = Request<Texture2D>("BombusApisBee/ExtraTextures/StarTexture").Value;
            for (int i = 0; i < 2; i++)
            {
    ...
    class TrueHomingNectar : BeeProjectile
    {
        ...
        public override void AI()
        {
            ...
            Dust.NewDustPerfect(Projectile.Center + Main.rand.NextVector2Circular(5f, 5f), DustType<GlowFastDecelerate>(), null, Scale: 0.55f, newColor: new Color(214, 158, 79));
            ...
            {
                Projectile.velocity = (Projectile.velocity * 20f + (targetCenter - Projectile.Center).SafeNormalize(Vector2.UnitX) * 20f) / 21f;
            }
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            for (int i = 0; i < 35; i++)
            {
                Dust.NewDustPerfect(Projectile.Center + Main.rand.NextVector2Circular(5f, 5f), DustType<GlowFastDecelerate>(), Main.rand.NextVector2Circular(4.5f, 4.5f), 0, new Color(255, 191, 73), 0.3f);
            }
        }
