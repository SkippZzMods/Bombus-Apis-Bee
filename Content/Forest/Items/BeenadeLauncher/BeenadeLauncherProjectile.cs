...
namespace BombusApisBee.Content.Forest.Items.BeenadeLauncher
{
    public class BeenadeLauncherProjectile : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Projectile.timeLeft <= 45)
            {
                for (int i = 0; i < 2; i++)
                {
                    ...
                    Dust.NewDustPerfect(Projectile.Center + circle, DustType<Glow>(), circle.DirectionTo(Projectile.Center), 0, Color.Orange, Main.rand.NextFloat(0.2f, 0.3f));
                }
            }
            else
                Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(), null, 0, Color.Orange, 0.35f);
        }
        ...
        {
            SoundEngine.PlaySound(new SoundStyle("BombusApisBee/Sounds/Item/FireCast"), Projectile.position);
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            new SoundStyle("BombusApisBee/Sounds/Item/FireHit").PlayWith(pos: Projectile.position, pitchVariance: 0.1f);
            for (int i = 0; i < 25; i++)
            {
                Dust.NewDustPerfect(Projectile.Center, DustType<Glow>(), (Projectile.rotation.ToRotationVector2() * Main.rand.NextFloat(10f)).RotatedByRandom(0.3f), 0, Color.Orange, Main.rand.NextFloat(0.8f, 0.95f));
            }
            for (int i = 0; i < 40; i++)
            {
                float angle = 6.2831855f * i / 40;
                Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(), angle.ToRotationVector2() * 1.45f, 0, Color.Orange, 0.5f);
            }
            ...
            for (int i = 0; i < 7; i++)
            {
                ...
                Dust.NewDustPerfect(Projectile.Center, DustType<ExplosionDust>(), velo.RotatedByRandom(0.3f) * Main.rand.NextFloat(5f, 25f), 70 + Main.rand.Next(60), default, Main.rand.NextFloat(0.9f, 1.5f)).rotation = Main.rand.NextFloat(6.28f);

                Dust.NewDustPerfect(Projectile.Center, DustType<ExplosionDustTwo>(), velo.RotatedByRandom(0.45f) * Main.rand.NextFloat(5f, 25f), Main.rand.Next(80) + 40, default, Main.rand.NextFloat(0.9f, 1.5f)).rotation = Main.rand.NextFloat(6.28f);

                Dust.NewDustPerfect(Projectile.Center, DustType<ExplosionDust>(), velo.RotatedByRandom(0.45f) * Main.rand.NextFloat(10f, 30f), 80 + Main.rand.Next(50), default, Main.rand.NextFloat(0.9f, 1.5f)).rotation = Main.rand.NextFloat(6.28f);
            }

            if (Main.myPlayer == Projectile.owner)
                Projectile.NewProjectileDirect(Projectile.GetSource_Death(), Projectile.Center, Vector2.Zero, ProjectileType<BasicExplosion>(), (int)(Projectile.damage * 1.5f), 2f, Projectile.owner, 80);
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Texture2D glowTex = Request<Texture2D>(Texture + "_Glowy").Value;
