...
namespace BombusApisBee.Content.Forest.Items.BeenadeLauncher
{
    public class BeenadeLauncher : BeekeeperWeapon
    {
        ...
        public override void SafeSetDefaults()
        {
            ...
            Item.autoReuse = true;
            Item.shoot = ProjectileType<BeenadeLauncherProjectile>();
            Item.shootSpeed = 16f;
        ...
        public override void UseStyle(Player player, Rectangle heldItemFrame)
        {
            ...
            if (animProgress < 0.3f)
            {
                ...
                itemPosition += itemRotation.ToRotationVector2() * MathHelper.Lerp(0f, -10f, EaseFunction.EaseQuinticOut.Ease(lerper));
            }
            else
            {
                float lerper = (animProgress - 0.3f) / 0.7f;
                itemPosition += itemRotation.ToRotationVector2() * MathHelper.Lerp(-10f, 0f, EaseFunction.EaseBackInOut.Ease(lerper));
            }
        ...
        public override void UseItemFrame(Player player)
        {
            ...
            if (animProgress < 0.15f)
            {
                ...
                rotation += MathHelper.Lerp(0f, -0.75f, EaseFunction.EaseQuinticOut.Ease(lerper)) * player.direction;
            }
            else
            {
                float lerper = (animProgress - 0.15f) / 0.85f;
                rotation += MathHelper.Lerp(-0.75f, 0, EaseFunction.EaseBackInOut.Ease(lerper)) * player.direction;
            }
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                Dust.NewDustPerfect(visualPosition, DustType<Dusts.ImpactLineDust>(), velocity.RotatedByRandom(0.3f) * Main.rand.NextFloat(0.2f, 1f), 0, new Color(255, 55, 20, 0), 0.12f);

                Dust.NewDustPerfect(visualPosition, DustType<Dusts.GlowFastDecelerate>(), velocity.RotatedByRandom(0.3f) * Main.rand.NextFloat(0.2f, 0.5f), 0, new Color(255, 55, 20), 0.5f);
            }
            ...
            for (int i = 0; i < 2; i++)
            {
                Dust dust = Dust.NewDustPerfect(visualPosition, DustType<HellfireBeemstickSmokeDust>(), Main.rand.NextVector2Circular(2f, 2f), 50, default, 0.1f);
                dust.rotation = Main.rand.NextFloat(6.28f);

                dust = Dust.NewDustPerfect(visualPosition, DustType<HellfireBeemstickSmokeDust>(), Vector2.UnitY * -1.5f, 50, default, 0.1f);
                dust.rotation = Main.rand.NextFloat(6.28f);
            }
