...
namespace BombusApisBee.Content.Forest.Items.HoneyFlareCannon
{
    public class HoneyFlareCannon : BeekeeperWeapon
    {
        ...
        {
            CreateRecipe(1).AddIngredient(ItemType<HoneyFlareGun.HoneyFlareGun>(), 1).AddIngredient(ItemID.HallowedBar, 14).AddTile(TileID.MythrilAnvil).Register();
        }
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            for (int i = 0; i < 15; i++)
            {
                Dust.NewDustPerfect(position, DustType<Dusts.HoneyDust>(), velocity.RotatedByRandom(0.45f) * Main.rand.NextFloat(0.25f), Main.rand.Next(100)).noGravity = true;
