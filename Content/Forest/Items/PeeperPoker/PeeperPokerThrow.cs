    ...
    public class PeeperPokerThrow : BeeProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Projectile.ai[0] > 5f && Main.myPlayer == Projectile.owner)
            {
                ...
                for (int i = 0; i < 3; i++)
                {
                    ...
                    Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.oldPosition, vel, ProjectileType<CthulhuBee>(), Projectile.damage * 1 / 3, Projectile.knockBack, Projectile.owner);
                    const int Repeats = 30;
                    for (int d = 0; d < Repeats; ++d)
                    {
                        float angle2 = 6.2831855f * d / Repeats;
                        Dust dust3 = Dust.NewDustPerfect(Projectile.Center, DustID.Blood, null, 0, default, 1.1f);
                        dust3.velocity = angle2.ToRotationVector2() * 4f;
                        dust3.noGravity = true;
                    }
                    const int Repeats2 = 45;
                    for (int d2 = 0; d2 < Repeats2; ++d2)
                    {
                        float angle2 = 6.2831855f * d2 / Repeats2;
                        Dust dust3 = Dust.NewDustPerfect(Projectile.Center, DustID.Blood, null, 0, default, 1.1f);
                        dust3.velocity = angle2.ToRotationVector2() * 4f;
                        dust3.noGravity = true;
                    }
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            if (Main.myPlayer == Projectile.owner)
            {
                ...
                for (int i = 0; i < 3; i++)
                {
                    Vector2 perturbedspeed = new Vector2(Projectile.velocity.X, Projectile.velocity.Y + Main.rand.Next(-3, 4)).RotatedBy((double)MathHelper.ToRadians(spread), default);
                    Projectile.NewProjectile(Projectile.GetSource_OnHit(target), Projectile.Center, perturbedspeed * 0.175f, ProjectileType<CthulhuBee>(), Projectile.damage * 1 / 3, 1f, Projectile.owner);
                }
            }
        }
