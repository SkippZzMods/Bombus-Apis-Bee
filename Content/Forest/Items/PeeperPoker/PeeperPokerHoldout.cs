...
namespace BombusApisBee.Content.Forest.Items.PeeperPoker
{
    public class PeeperPokerHoldout : BeeProjectile
    {
        ...
        public override void AI()
        {
            ...
            int expectedDirection = ((Main.MouseWorld - player.Center).SafeNormalize(fallback.Value).X > 0f).ToDirectionInt();
            float num = 50f;
            float num2 = 2f;
            ...
            Projectile.velocity = new Vector2(num4, 0f);
            if (Projectile.ai[0] == 0f)
            {
                Projectile.rotation = new Vector2(num4, -player.gravDir).ToRotation() + num3 + 3.1415927f;
            ...
            Projectile.rotation += 6.2831855f * num2 / num * num4;
            ...
            if (!OwnerCanShoot || modPlayer.BeeResourceCurrent <= 0 || flag2 && !player.controlUseItem)
            {
                ...
                if (Main.myPlayer == Projectile.owner && player.ownedProjectileCounts[ProjectileType<PeeperPokerThrow>()] < 2)
                {
                    SoundEngine.PlaySound(SoundID.Item1);
                    int Damage = (int)(Projectile.damage * DamageFloat);
                    int DamageAmt = Utils.Clamp(Damage, 0, 55);
                    ...
                    float VelocityFloat2 = Utils.Clamp(VelocityFloat, 1f, 3f);
