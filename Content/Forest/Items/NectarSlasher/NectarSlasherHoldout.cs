...
namespace BombusApisBee.Content.Forest.Items.NectarSlasher
{
    public class NectarSlasherHoldout : BeeProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Combo >= 2)
            {
                ...
                if (++throwTimer < maxThrowTimer)
                {
                    ...
                    Dust.NewDustPerfect(swordLength, DustType<Dusts.GlowFastDecelerate>(), null, 0, new Color(214, 158, 79, 50), 0.4f);
                }
                else
                {
                    ...
                    Dust.NewDustPerfect(Projectile.Center + Projectile.rotation.ToRotationVector2() * 20f + Projectile.velocity, DustType<Dusts.Glow>(), Vector2.Zero, Scale: 0.45f, newColor: new Color(214, 158, 79, 50));
            ...
            if (Main.rand.NextBool())
            {
                ...
                Dust.NewDustPerfect(swordLength, DustType<Dusts.GlowFastDecelerate>(), null, 0, new Color(214, 158, 79, 50), 0.35f);
            }
        }
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            for (int i = 0; i < 15; i++)
            {
                Dust.NewDustPerfect(Projectile.Center + Main.rand.NextVector2Circular(5f, 5f), DustType<Dusts.GlowFastDecelerate>(), Main.rand.NextVector2Circular(3.5f, 3.5f), 0, new Color(255, 191, 73), 0.3f);
            }

            Vector2 pos = target.Center + Main.rand.NextVector2CircularEdge(150f, 150f);
            Projectile.NewProjectile(Projectile.GetSource_OnHit(target), pos, pos.DirectionTo(target.Center) * 20f, ProjectileType<Nectarslash>(), Projectile.damage, 2f, Projectile.owner);
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Texture2D swordGlow = Request<Texture2D>(Texture + "_Glowy").Value;
