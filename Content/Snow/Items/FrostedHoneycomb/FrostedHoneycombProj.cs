namespace BombusApisBee.Content.Snow.Items.FrostedHoneycomb
{
    public class FrostedHoneycombProj : BeeProjectile
    {
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            Projectile.NewProjectile(Projectile.GetSource_Death(), Projectile.Center + Projectile.velocity, velo, ProjectileType<FrostedProjectile_Homing>(), (int)(Projectile.damage * 0.66f), 2f, Projectile.owner);
            velo = -Vector2.UnitY.RotatedByRandom(0.3f) * Main.rand.NextFloat(8f, 12f);
            Projectile.NewProjectile(Projectile.GetSource_Death(), Projectile.Center + Projectile.velocity, velo, ProjectileType<FrostedProjectile_Solid>(), Projectile.damage, 5f, Projectile.owner);
            velo = -Vector2.UnitY.RotatedByRandom(0.3f) * Main.rand.NextFloat(8f, 12f);
            Projectile.NewProjectile(Projectile.GetSource_Death(), Projectile.Center + Projectile.velocity, velo, ProjectileType<FrostedProjectile_Piercing>(), (int)(Projectile.damage * 0.75f), 1.5f, Projectile.owner);

            for (int i = 0; i < 25; i++)
            {
                Dust dust = Dust.NewDustPerfect(Projectile.Center + Main.rand.NextVector2Circular(10f, 10f), DustType<Dusts.FrostedDust>(), (velo * Main.rand.NextFloat(0.25f)).RotatedByRandom(0.3f));
                dust.noGravity = true;
                dust.velocity *= 0.5f;

                dust = Dust.NewDustPerfect(Projectile.Center + Main.rand.NextVector2Circular(15f, 15f), DustType<Dusts.FrostedDust>());
                dust.velocity *= 0.5f;

                Dust.NewDustPerfect(Projectile.Center + Main.rand.NextVector2Circular(15f, 15f), DustType<Dusts.GlowFastDecelerate>(), newColor: new Color(153, 212, 242)).scale = 0.4f;
            }
        }
    ...
    public class FrostedProjectile_Homing : BeeProjectile
    {
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                Dust dust = Dust.NewDustPerfect(Projectile.Center + Main.rand.NextVector2Circular(5f, 5f), DustType<Dusts.FrostedDust>());
                dust.noGravity = true;
                dust.velocity *= 0.5f;
                Dust.NewDustPerfect(Projectile.Center + Main.rand.NextVector2Circular(7f, 7f), DustType<Dusts.FrostedDust>()).velocity *= 0.5f;

                Dust.NewDustPerfect(Projectile.Center + Main.rand.NextVector2Circular(5f, 5f), DustType<Dusts.GlowFastDecelerate>(), newColor: new Color(153, 212, 242)).scale = 0.3f;
            }
        }
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
