...
namespace BombusApisBee.Content.MechBoss.Items.HoneycombCrusaderArmor
{
    ...
    public class HoneycombCrusaderHelmet : BeeKeeperItem
    {
        ...
        {
            return legs.type == ItemType<HoneycombCrusaderGreaves>() && body.type == ItemType<HoneycombCrusaderPlatemail>();
        }
    ...
    class HoneycombCrusaderPlayer : ModPlayer
    {
        ...
        public override void PostUpdateMiscEffects()
        {
            if (FullArmorSet && Player.ownedProjectileCounts<HoneycombCrusaderShieldProjectile>() <= 0)
                Projectile.NewProjectile(Player.GetSource_Misc("BombusApisBee: Spawn Holy Shield"), Player.Center, Vector2.Zero, ProjectileType<HoneycombCrusaderShieldProjectile>(), Player.ApplyHymenoptraDamageTo(100), 0f, Player.whoAmI);
        }
    ...
    class HoneycombCrusaderShieldProjectile : ModProjectile
    {
        ...
        public override void AI()
        {
            if (deathTimer > 0)
            {
                if (deathTimer == 1)
                {
                    ...
                    BeeUtils.CircleDust(collisionPoint(owner), 30, DustType<GlowFastDecelerate>(), 2f, 0, new Color(150, 100, 30), 0.5f);
                }
                deathTimer--;
            }
            ...
            if (Main.rand.NextBool(5) && deathTimer <= 0)
                Dust.NewDustPerfect(collisionPoint(owner) + Main.rand.NextVector2Circular(15f, 15f), DustType<GlowFastDecelerate>(), Vector2.Zero, 0, new Color(155, 105, 20) * (1f - deathTimer / 600f), Main.rand.NextFloat(0.3f, 0.6f));
            ...
            for (int i = 0; i < Main.maxProjectiles; i++)
            {
                ...
                if (deathTimer <= 0 && hitTimer <= 0 && Vector2.Distance(collisionPoint(owner), proj.Center) < 50f && proj.active && proj.hostile && proj.damage > 0 && owner.GetModPlayer<HoneycombCrusaderPlayer>().hitProjHitTimer[proj.whoAmI] <= 0)
                {
                    for (int x = 0; x < 10; x++)
                    {
                        Dust.NewDustPerfect(proj.Center + Main.rand.NextVector2Circular(5f, 5f), DustType<GlowFastDecelerate>(), Main.rand.NextVector2Circular(4.5f, 4.5f), 0, new Color(165, 105, 0), 0.35f);

                        Dust.NewDustPerfect(owner.Center + owner.Center.DirectionTo(proj.Center) * 65f, DustType<GlowFastDecelerate>(), (owner.Center.DirectionTo(proj.Center) * 5f).RotatedByRandom(0.4f) * Main.rand.NextFloat(0.25f, 1.75f), 0, new Color(150, 50, 20), 0.55f);

                        Dust.NewDustPerfect(owner.Center + owner.Center.DirectionTo(proj.Center) * 65f, DustType<GlowFastDecelerate>(), (owner.Center.DirectionTo(proj.Center) * 5f).RotatedByRandom(1f) * Main.rand.NextFloat(0.25f, 1f), 0, new Color(200, 150, 20), 0.75f);
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            effect.Parameters["uImage2"].SetValue(Request<Texture2D>("BombusApisBee/ExtraTextures/MiscNoise1").Value);
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            for (int x = 0; x < 10; x++)
            {
                Dust.NewDustPerfect(target.Center + Main.rand.NextVector2Circular(5f, 5f), DustType<GlowFastDecelerate>(), Main.rand.NextVector2Circular(4.5f, 4.5f), 0, new Color(165, 105, 0), 0.35f);

                Dust.NewDustPerfect(owner.Center + owner.Center.DirectionTo(target.Center) * 65f, DustType<GlowFastDecelerate>(), (owner.Center.DirectionTo(target.Center) * 5f).RotatedByRandom(0.4f) * Main.rand.NextFloat(0.25f, 1.75f), 0, new Color(150, 50, 20), 0.55f);

                Dust.NewDustPerfect(owner.Center + owner.Center.DirectionTo(target.Center) * 65f, DustType<GlowFastDecelerate>(), (owner.Center.DirectionTo(target.Center) * 5f).RotatedByRandom(1f) * Main.rand.NextFloat(0.25f, 1f), 0, new Color(200, 150, 20), 0.75f);
        ...
        internal void DropChunks()
        {
            ...
            {
                Item item = Main.item[Item.NewItem(Projectile.GetSource_FromAI(), Projectile.getRect(), ItemType<HoneycombChunkPickup>())];
