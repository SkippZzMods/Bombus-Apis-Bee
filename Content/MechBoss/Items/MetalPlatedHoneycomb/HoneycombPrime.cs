...
namespace BombusApisBee.Content.MechBoss.Items.MetalPlatedHoneycomb
{
    public class HoneycombPrime : BeeProjectile
    {
        ...
        public override string Texture => "BombusApisBee/Content/MechBoss/Items/MetalPlatedHoneycomb/MetalPlatedHoneycomb";
        ...
        public override void AI()
        {
            ...
            {
                Projectile proj = Projectile.NewProjectileDirect(Projectile.GetSource_FromThis(), Projectile.Center, Vector2.UnitY, ProjectileType<HoneycombPrime_Saw>(), Projectile.damage * 2, 5f, Projectile.owner);
                (proj.ModProjectile as HoneycombPrime_Saw).parent = Projectile;

                Projectile proj2 = Projectile.NewProjectileDirect(Projectile.GetSource_FromThis(), Projectile.Center, -Vector2.UnitY, ProjectileType<HoneycombPrime_Laser>(), Projectile.damage, 2.5f, Projectile.owner);
            ...
            if (++shootDelay >= maxShootDelay)
            {
                if (Main.myPlayer == Projectile.owner)
                {
                    ...
                    for (int i = 0; i < 15; i++)
                    {
                        Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(), velocity.RotatedByRandom(0.35f) * Main.rand.NextFloat(2.5f, 4.5f), 0, new Color(213, 95, 89), Main.rand.NextFloat(0.35f, 0.4f));
                        Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(), velocity.RotatedByRandom(0.4f) * Main.rand.NextFloat(2f, 4f), 0, new Color(161, 31, 85), Main.rand.NextFloat(0.5f, 0.5f));
                    }

                    Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, velocity * 6f, ProjectileType<MetalBee>(), Projectile.damage, 0f, Projectile.owner);
            ...
            Dust.NewDustPerfect(Main.GetPlayerArmPosition(Projectile) + Main.rand.NextVector2Circular(5f, 5f), DustType<Glow>(), Vector2.Zero, 0, new Color(213, 95, 89, 50), 0.4f);
        }
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            for (int i = 0; i < 15; i++)
            {
                Dust.NewDustPerfect(Projectile.Center + Main.rand.NextVector2Circular(5f, 5f), DustType<Dusts.GlowFastDecelerate>(), Main.rand.NextVector2Circular(2f, 2f), 55,
    ...
    class HoneycombPrime_Saw : BeeProjectile
    {
        ...
        public override void AI()
        {
            ...
            else
            {
                if (!thrown)
                {
                    ...
                    for (int i = 0; i < 35; i++)
                    {
                        Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(), Projectile.velocity.RotatedByRandom(0.4f) * Main.rand.NextFloat(0.5f), 0, new Color(213, 95, 89), Main.rand.NextFloat(0.35f, 0.4f));

                        Dust.NewDustPerfect(Projectile.Center + Main.rand.NextVector2Circular(5f, 5f), DustType<GlowFastDecelerate>(), Main.rand.NextVector2Circular(5f, 5f), 0, new Color(213, 95, 89), Main.rand.NextFloat(0.45f, 0.5f));
                    }
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            for (int i = 0; i < 15; i++)
            {
                Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(), Projectile.velocity.RotatedByRandom(0.4f) * -Main.rand.NextFloat(0.25f), 0, new Color(213, 95, 89), Main.rand.NextFloat(0.35f, 0.4f));
            }
        }
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                Dust.NewDustPerfect(Projectile.Center + Main.rand.NextVector2Circular(5f, 5f), DustType<Dusts.GlowFastDecelerate>(), Main.rand.NextVector2Circular(1.25f, 1.25f), 55,
                    Main.rand.NextBool() ? new Color(213, 95, 89) : new Color(161, 31, 85), Main.rand.NextFloat(0.5f, 0.6f));
            }
    ...
    class HoneycombPrime_Laser : BeeProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (++charge >= MAXCHARGE && shots < 3)
            {
                if (Main.myPlayer == Projectile.owner)
                {
                    if (++shootDelay > MAXSHOOTDELAY)
                    {
                        ...
                        for (int i = 0; i < 25; i++)
                        {
                            Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(), velocity * Main.rand.NextFloat(4.5f), 0, new Color(213, 95, 89), Main.rand.NextFloat(0.35f, 0.4f));
                        }

                        for (int i = 0; i < 25; ++i)
                        {
                            float angle2 = 6.28f * i / 25;
                            Dust.NewDustPerfect(Projectile.Center, DustType<Dusts.GlowFastDecelerate>(), angle2.ToRotationVector2() * 0.75f, 0, new Color(161, 31, 85), 0.35f);
                        }

                        Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center, velocity * 8f,
                            ProjectileType<RedLaser>(), Projectile.damage, 2.5f, Projectile.owner);
            ...
            else if (shots >= 3)
            {
                ...
                {
                    Dust.NewDustPerfect(Projectile.Center + Projectile.rotation.ToRotationVector2() * 7f, DustType<GlowFastDecelerate>(), Projectile.rotation.ToRotationVector2().RotatedByRandom(0.4f) * Main.rand.NextFloat(6f), newColor: new Color(213, 95, 89, 50)).scale = 0.4f;
                    if (!laser)
                    {
                        if (Projectile.soundDelay <= 0)
                        {
            ...
            if (laser)
            {
                ...
                if (++laserTimer < MAXLASERTIMER)
                {
                    ...
                    Dust.NewDustPerfect(Vector2.Lerp(Projectile.Center, Projectile.Center + Vector2.One.RotatedBy(Projectile.rotation - MathHelper.PiOver4) * 500f, Main.rand.NextFloat()),
                        DustType<GlowFastDecelerate>(), newColor: Main.rand.NextBool() ? new Color(213, 95, 89) : new Color(161, 31, 85), Scale: 0.35f).velocity *= 0.5f;
