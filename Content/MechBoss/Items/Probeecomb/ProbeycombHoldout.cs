...
namespace BombusApisBee.Content.MechBoss.Items.Probeecomb
{
    public class ProbeycombHoldout : BeeProjectile
    {
        ...
        public override string Texture => "BombusApisBee/Content/MechBoss/Items/Probeecomb/ProbeyComb";
        ...
        public override void AI()
        {
            ...
            Dust.NewDustPerfect(dustPos, DustType<GlowFastDecelerate>(), Vector2.Zero, newColor: new Color(213, 95, 89, 50)).scale = 0.3f;
        }
        ...
        public void ShootThings()
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(), Projectile.velocity.RotatedByRandom(0.35f) * Main.rand.NextFloat(2.5f, 5.5f), 0, new Color(213, 95, 89, 50), Main.rand.NextFloat(0.25f, 0.55f));
                Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(), Projectile.velocity.RotatedByRandom(0.3f) * Main.rand.NextFloat(2f, 5f), 0, new Color(161, 31, 85, 50), Main.rand.NextFloat(0.3f, 0.6f));
            }

            if (Main.myPlayer == Projectile.owner)
                Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, Projectile.velocity * 6f, ProjectileType<Probee>(), Projectile.damage, 0f, Projectile.owner);
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Texture2D texGlow = Request<Texture2D>("BombusApisBee/Content/MechBoss/Items/Probeecomb/ProbeycombHoldout_Glow").Value;
