namespace BombusApisBee.Content.MechBoss.Items.OcularRemote
{
    public class RetinaHoneycomb : BeeProjectile
    {
            ...
            if (deathAnimation)
            {
                ...
                if (++attackDelay < 120)
                {
                    ...
                    for (int i = 0; i < 5; i++)
                    {
                        ...
                        Dust.NewDustPerfect(pos + Main.rand.NextVector2CircularEdge(lerper, lerper), DustType<Dusts.GlowFastDecelerate>(), Vector2.Zero, 0, new Color(161, 31, 85), 0.4f);
                    }
                    ...
                    if (attackDelay % (int)MathHelper.Lerp(10, 3, attackDelay / 120f) == 0)
                    {
                        ...
                        for (int i = 0; i < 35; ++i)
                        {
                            float angle2 = 6.28f * i / 35;
                            Dust.NewDustPerfect(pos, DustType<Dusts.GlowFastDecelerate>(), angle2.ToRotationVector2() * 1.25f, 0, new Color(161, 31, 85), 0.35f);
                        }

                        if (Main.myPlayer == Projectile.owner)
                            Projectile.NewProjectile(Projectile.GetSource_FromAI(), pos, velo * 8f,
                                ProjectileType<RedLaserHoming>(), Projectile.damage / 2, 2.5f, Projectile.owner);
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            for (int i = 0; i < 55; i++)
            {
                Dust.NewDustPerfect(Projectile.Center + Main.rand.NextVector2Circular(5f, 5f), DustType<Dusts.GlowFastDecelerate>(), Main.rand.NextVector2Circular(4.25f, 4.25f), 0,
                    Main.rand.NextBool() ? new Color(213, 95, 89) : new Color(161, 31, 85), Main.rand.NextFloat(0.7f, 0.95f));
            }

            for (int i = 0; i < 45; ++i)
            {
                float angle2 = 6.28f * i / 45;
                Dust.NewDustPerfect(Projectile.Center, DustType<Dusts.GlowFastDecelerate>(), angle2.ToRotationVector2() * 4.25f, 0, new Color(161, 31, 85), 0.85f);
            }

            if (Main.myPlayer == Projectile.owner)
                for (int i = 0; i < 5; i++)
                {
                    Projectile.NewProjectile(Projectile.GetSource_Death(), Projectile.Center, Main.rand.NextVector2Circular(3f, 3f), ProjectileType<MetalBee>(), Projectile.damage / 2, 0f, Projectile.owner);
                }
    ...
    public class SpazHoneycomb : BeeProjectile
    {
            ...
            if (target != default)
            {
                if (dashes < 3)
                {
                    ...
                    if (DashDelay <= 0 && Projectile.Distance(target.Center) < 450f)
                    {
                        for (int i = 0; i < 25; i++)
                        {
                            Dust.NewDustPerfect(Projectile.Center + Main.rand.NextVector2Circular(15f, 15f), DustType<GlowFastDecelerate>(), Main.rand.NextVector2Circular(2f, 2f), 0, new Color(98, 205, 123), 0.65f);

                            Dust.NewDustPerfect(Projectile.Center + Main.rand.NextVector2Circular(15f, 15f), DustType<GlowFastDecelerate>(), Main.rand.NextVector2Circular(3.5f, 3.5f), 0, new Color(231, 215, 157), 0.5f);
                        }

                        for (int i = 0; i < Main.rand.Next(1, 4); i++)
                        {
                            Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center, Main.rand.NextVector2Circular(3f, 3f), ProjectileType<SpazBee>(), Projectile.damage * 2 / 3, 2f, Projectile.owner);
                        }
                ...
                else
                {
                    ...
                    if (ChargeDelay < 100)
                    {
                        ...
                        Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(),
                    ...
                    else
                    {
                        if (Main.myPlayer == Projectile.owner && ChargeDelay == 100)
                        {
                            for (int i = 0; i < 25; i++)
                            {
                                Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(), Projectile.rotation.ToRotationVector2().RotatedByRandom(0.55f) * Main.rand.NextFloat(4.5f, 6.5f), 0, new Color(98, 205, 123), Main.rand.NextFloat(0.45f, 0.6f));
                                Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(), Projectile.rotation.ToRotationVector2().RotatedByRandom(0.6f) * Main.rand.NextFloat(4f, 6f), 0, new Color(231, 215, 157), Main.rand.NextFloat(0.6f, 0.7f));
                            }
                            Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center, Projectile.rotation.ToRotationVector2() * 15f, ProjectileType<CursedFlameball>(), Projectile.damage, 3f, Projectile.owner);
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            if (Main.myPlayer == Projectile.owner)
                Projectile.NewProjectile(Projectile.GetSource_Death(), Projectile.Center, Vector2.Zero, ProjectileType<CursedExplosion>(), Projectile.damage, 0f, Projectile.owner, 95);

            for (int i = 0; i < 35; i++)
            {
                Dust.NewDustPerfect(Projectile.Center + Main.rand.NextVector2Circular(15f, 15f), DustType<GlowFastDecelerate>(), Main.rand.NextVector2Circular(7f, 7f), 0, new Color(98, 205, 123), 0.8f);

                Dust.NewDustPerfect(Projectile.Center + Main.rand.NextVector2Circular(15f, 15f), DustType<GlowFastDecelerate>(), Main.rand.NextVector2Circular(10.5f, 10.5f), 0, new Color(231, 215, 157), 0.7f);
            }
        }
