...
namespace BombusApisBee.Content.Martian.Items.LazerbeemBlaster
{
    public class LaserbeemBlaster : BeeDamageItem
    {
        ...
        public override void SafeSetDefaults()
        {
            ...
            Item.shoot = ProjectileType<LaserbeemProjectile>();
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            for (int i = 0; i < 15; i++)
            {
                Dust.NewDustPerfect(position, DustType<Dusts.Glow>(), velocity.RotatedByRandom(0.25f) * Main.rand.NextFloat(1f), 0, new Color(255, 255, 204), 0.35f);

                Dust.NewDustPerfect(position, DustType<Dusts.GlowFastDecelerate>(), velocity.RotatedByRandom(0.25f) * Main.rand.NextFloat(0.75f), 0, new Color(253, 232, 0), 0.45f);
            }
            return true;
        }
