...
namespace BombusApisBee.Content.Martian.Items.Beenopopper
{
    public class BeeBubbleBlaster : BeeDamageItem
    {
        ...
        public override void SafeSetDefaults()
        {
            ...
            Item.autoReuse = true;
            Item.shoot = ProjectileType<BeeBubble>();
            Item.shootSpeed = 5;
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            for (int i = 0; i < 15; i++)
            {
                Dust.NewDustPerfect(position + Vector2.Normalize(velocity) * 35f, DustType<Dusts.HoneyDust>(), velocity.RotatedByRandom(0.4f) * Main.rand.NextFloat(3f), 0, default, 1.2f).noGravity = true;
