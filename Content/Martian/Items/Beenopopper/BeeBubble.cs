namespace BombusApisBee.Content.Martian.Items.Beenopopper
{
    public class BeeBubble : BeeProjectile
    {
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            {
                Dust dust = Dust.NewDustDirect(Projectile.position, Projectile.width, Projectile.height, DustType<HoneyDust>());
                dust.noGravity = true;
                dust.scale = 0.8f;
                dust.alpha = 150;
            }
        ...
        public override void AI()
        {
            ...
            if (Projectile.timeLeft == 0 && Projectile.owner == Main.myPlayer)
            {
                ...
                {
                    value3.Y = Main.screenHeight - Main.mouseY + Main.screenPosition.Y;
                }
                Vector2 vector3 = Vector2.Normalize(value3 - Projectile.Center);
                vector3 *= 18;
                Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center, vector3, ProjectileType<BeeBullet>(), Projectile.damage, Projectile.knockBack, Projectile.owner);
