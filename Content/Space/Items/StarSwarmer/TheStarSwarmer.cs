    ...
    public class TheStarSwarmer : BeekeeperWeapon
    {
        ...
        public override void HoldItem(Player player)
        {
            if (cooldown > 0)
            {
                if (cooldown == 1)
                {
                    ...
                    BeeUtils.DrawStar(player.GetArmPosition(), ModContent.DustType<GlowFastDecelerate>(), 0, new Color(181, 127, 207), true, 5, 1.25f, 1f, 0.4f, 0.3f);
                }
                cooldown--;
            }
        }
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            if (player.altFunctionUse == 2)
            {
                ...
                for (int i = 0; i < 20; i++)
                {
                    Dust.NewDustPerfect(pos, ModContent.DustType<GlowFastDecelerate>(), velocity.RotatedByRandom(0.35f) * Main.rand.NextFloat(2f), 0, new Color(181, 127, 207), 0.45f);

                    Dust.NewDustPerfect(pos, ModContent.DustType<GlowFastDecelerate>(), velocity.RotatedByRandom(0.35f) * Main.rand.NextFloat(1f), 0, new Color(112, 83, 163), 0.4f);
                }
                for (int i = 0; i < 12; i++)
                {
                    Dust.NewDustPerfect(pos, ModContent.DustType<Stardust>(), velocity.RotatedByRandom(0.3f) * Main.rand.NextFloat(1.25f), 0, default, 1.15f);
                }
                ...
                for (float k = 0; k < 6.28f; k += 0.1f)
                {
                    ...
                    Dust.NewDustPerfect(pos, ModContent.DustType<GlowFastDecelerate>(), new Vector2(x, y).RotatedBy(velocity.ToRotation() + MathHelper.PiOver2) * 0.05f, 0, new Color(112, 83, 163), 0.45f);
                }
            ...
            else
            {
                ...
                for (int i = 0; i < 15; i++)
                {
                    Dust.NewDustPerfect(pos, ModContent.DustType<GlowFastDecelerate>(), velocity.RotatedByRandom(0.3f) * Main.rand.NextFloat(2f), 0, new Color(181, 127, 207), 0.45f);

                    Dust.NewDustPerfect(pos, ModContent.DustType<GlowFastDecelerate>(), velocity.RotatedByRandom(0.3f) * Main.rand.NextFloat(1f), 0, new Color(107, 172, 255), 0.4f);
                }
                for (int i = 0; i < 8; i++)
                {
                    Dust.NewDustPerfect(pos, ModContent.DustType<Stardust>(), velocity.RotatedByRandom(0.3f) * Main.rand.NextFloat(2.25f), 0, default, 1.15f);
                }
            }
            return true;
        }
        public override void AddRecipes()
        {
            ...
            CreateRecipe(1).AddIngredient(ModContent.ItemType<TheStarStrap>()).
                ...
                AddIngredient(ModContent.ItemType<PollenItem>(), 15).
