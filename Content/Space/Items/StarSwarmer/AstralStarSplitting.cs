...
namespace BombusApisBee.Content.Space.Items.StarSwarmer
{
    public class AstralStarSplitting : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (split)
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    ...
                    Dust.NewDustPerfect(pos, DustType<GlowFastDecelerate>(), pos.DirectionTo(Projectile.Center) * 0.25f, 0, new Color(181, 127, 207), 0.35f);
                }
            ...
            //dust
            Dust.NewDustPerfect(Projectile.Center + Projectile.velocity * 1.5f, DustType<GlowFastDecelerate>(), Vector2.UnitX.RotatedBy(Projectile.velocity.ToRotation() - MathHelper.PiOver2) * 1.85f, 0, new Color(181, 127, 207), 0.35f);
            Dust.NewDustPerfect(Projectile.Center + Projectile.velocity * 1.5f, DustType<GlowFastDecelerate>(), -Vector2.UnitX.RotatedBy(Projectile.velocity.ToRotation() - MathHelper.PiOver2) * 1.85f, 0, new Color(181, 127, 207), 0.35f);
        }
        ...
        public override void OnKill(int timeLeft)
        {
            if (split)
            {
                BeeUtils.DrawStarWithRot(Projectile.Center, DustType<GlowFastDecelerate>(), 0, new Color(181, 127, 207), true, 5, 2.25f, 1f, 0.6f, 0.35f, rot: Projectile.rotation);
                if (Main.myPlayer == Projectile.owner && !Projectile.noDropItem)
                {
                    int item = Item.NewItem(Projectile.GetSource_DropAsItem(), Projectile.Hitbox, ItemType<AstralStarPickup>());
        ...
        public void SplitEffects(Vector2 velocity)
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                Projectile proj = Projectile.NewProjectileDirect(Projectile.GetSource_Death(), Projectile.Center, velocity.RotatedByRandom(0.3f) * Main.rand.NextFloat(0.75f, 1.25f), ProjectileType<AstralStarSplittingShards>(), Projectile.damage / 5, 1f, Projectile.owner);
            ...
            for (int i = 0; i < 25; i++)
            {
                Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(), Main.rand.NextVector2Circular(5.5f, 5.5f), 0, new Color(181, 127, 207), 0.65f);

                Dust.NewDustPerfect(Projectile.Center, DustType<Glow>(), velocity.RotatedByRandom(0.3f) * Main.rand.NextFloat(0.4f), 0, new Color(181, 127, 207), 0.4f);

                Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(), velocity.RotatedByRandom(0.4f) * Main.rand.NextFloat(0.75f), 0, new Color(112, 83, 163), 0.4f);
            }

            for (int i = 0; i < 8; i++)
            {
                Dust.NewDustPerfect(Projectile.Center, DustType<Stardust>(), velocity.RotatedByRandom(0.3f) * Main.rand.NextFloat(0.55f), 0, default, 1.15f);
            }
    ...
    class AstralStarSplittingShards : ModProjectile
    {
        ...
        public override void OnKill(int timeLeft)
        {
            for (int i = 0; i < 8; i++)
            {
                Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(), Main.rand.NextVector2Circular(2.5f, 2.5f), 0, new Color(181, 127, 207), 0.35f);

                Dust.NewDustPerfect(Projectile.Center, DustType<GlowFastDecelerate>(), Main.rand.NextVector2Circular(2.5f, 2.5f) * Main.rand.NextFloat(0.75f), 0, new Color(112, 83, 163), 0.2f);
            }

            for (int i = 0; i < 3; i++)
            {
                Dust.NewDustPerfect(Projectile.Center, DustType<Stardust>(), Main.rand.NextVector2Circular(2.5f, 2.5f) * Main.rand.NextFloat(0.55f), 0, default, 0.8f);
            }
        }
